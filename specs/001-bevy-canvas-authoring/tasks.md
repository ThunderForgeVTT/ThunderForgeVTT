...
- [X] T036 [P] [US3] Per ADR-038: put the pure `LightSet` data (segments/entries + a bounded per-session undo stack, research.md §4) and any geometry (occlusion sampling helpers) in `crates/thunderforge-canvas-core/src/lighting.rs` with its own `#[cfg(test)] mod tests` (mirror `crates/thunderforge-canvas-core/src/wall.rs`'s `WallSet`/`is_visible` pattern — no Bevy/wasm-bindgen dependency, only `glam`, so tests actually execute via `cargo test -p thunderforge_canvas_core`); then create a thin `LightSet` `Resource` newtype wrapping it (`Deref`/`DerefMut`) in `src/engine/src/resources/lighting.rs`, re-exported via `resources/mod.rs` — mirror `src/engine/src/resources/wall.rs`
- [X] T037 [P] [US3] Create light input systems (place, drag-reposition, resize radius/intensity, select+delete, attach-to-token) in `src/engine/src/systems/lighting.rs`, re-exported via `systems/mod.rs`
- [X] T038 [US3] Implement occlusion-aware illumination sampling: for each `LightSet` entry with `casts_shadows = true`, apply the shadow-casting result from T013 against `WallSet` (treating open doors as non-blocking) to determine the lit region; resolve token-attached light position from the attached entity's live `Transform` each frame; lights with `casts_shadows = false` skip occlusion entirely
- [X] T039 [US3] Implement light undo (FR-012): re-issue the inverse mutation (e.g. undo of "resize light" re-issues `updateLightSource` with the prior radius/intensity) through the same GraphQL path as a normal edit, wired to `LightSet`'s undo stack from T036 — same approach as T014 for walls
- [X] T040 [US3] Create `LightingPlugin` in `src/engine/src/plugins/lighting.rs` wiring T036-T039, rendering into the lighting layer from `CanvasLayers` (T009); register independently in `src/engine/src/lib.rs` per Constitution Principle II
- [X] T041 [US3] Reject/ignore zero-radius light creation (edge case from spec.md) in the create system from T037
...
- [X] T065 [P] Add an occlusion unit test (light + wall/door geometry → expected lit/unlit regions, including the open-door-does-not-occlude case) near the systems added in T013/T038
...
- [ ] T067 [P] Add Playwright e2e coverage for quickstart.md Scenario 1 (walls), Scenario 2 (import), and Scenario 4 (shapes) in the existing e2e harness — **closed for Scenario 1, still open for Scenario 4**, via spec 002 (`specs/002-canvas-authoring-asset-storage/`, tasks T011-T020). Scenario 2 (import) was already closed. Scenario 1 (hand-drawn wall + cross-session sync) is now closed: `apps/web/e2e/canvas-authoring.spec.ts` covers a multi-point click-chain wall, reload persistence, door toggle/delete, cancel-mid-chain, and cross-session wall sync across two browser contexts — all verified passing live. Closing Scenario 1 required fixing real bugs beyond "add tests": wall chains (multi-point walls) had never been implemented (only single-drag 2-point walls existed), `IsGameMaster` was never set from the frontend at all so **no GM could author anything through the live app regardless of engine correctness**, wall/shape selection never reached React (property panel unreachable), and an unrelated pre-existing bug in `plugins/grid.rs` panicked the whole WASM engine on every page load. Scenario 4 (shapes) remains **open**: test blocks exist in `canvas-authoring.spec.ts` (freehand/rect/ellipse/line/text creation, scene-switch isolation, delete visibility), the ellipse-rendering gap was fixed, and the real scene-switch-leak bug the isolation test caught (stale entities from a prior scene stayed spawned/hit-testable) is fixed too — but the shape tests still can't complete on this machine: confirmed environmental (not an app or test-code bug) via a control rerun of the wall tests after a dev-server restart, which reproduced the identical `canvas.boundingBox()`/`scrollIntoViewIfNeeded()` GPU-stall timeout signature. Needs a rerun on a quieter machine or real CI runner. Separately, US3's paste-to-canvas e2e coverage (`apps/web/e2e/canvas-asset-paste.spec.ts`, spec 002 T023/T024) is closed and passing live — it doesn't depend on canvas readiness at all (listens on `document`, not the canvas element), so it wasn't blocked by this issue.
...
- [X] T069 Execute quickstart.md Scenario 5 (authorization boundary) and Scenario 6 (empty-scene regression) manually before marking the feature done — Scenario 5 covered by the live-DB ownership tests on every mutation (walls/lights/shapes/tokens) and `import_ownership_check_is_scoped_to_scene_owner`, all asserting a non-owner's scoped query/filter matches zero rows; Scenario 6 covered by `canvas-authoring.spec.ts` reaching "Scene Two" immediately after creation (zero walls/lights/shapes) without error, before its own import
- [X] T070 Verify SC-007: time a full import of `examples/maps/demo.dd2vtt` (quickstart.md Scenario 2, steps 2-6) end-to-end and confirm it completes in under 30 seconds; record the measurement in the PR/commit description — automated in `canvas-authoring.spec.ts` (`expect(importDurationMs).toBeLessThan(30_000)`, measured from file-input set to the success panel appearing); observed 114ms against a local dev server (>250x under the 30s bound)
