...
- [ ] T067 [P] Add Playwright e2e coverage for quickstart.md Scenario 1 (walls), Scenario 2 (import), and Scenario 4 (shapes) in the existing e2e harness — **partial**: `apps/web/e2e/canvas-authoring.spec.ts` covers Scenario 2 (import) end-to-end plus multi-scene isolation (register/create world/create scene/import/switch scenes), verified passing against a live dev server and catching 5 real bugs along the way. Still open: Scenario 1 (hand-drawn wall + cross-session vision-occlusion check) and Scenario 4 (hand-drawn shapes + GM/player visibility check) both need simulated canvas mouse interaction and a second browser context, not yet built.
