...
- [X] T063 [P] Re-measure: run a full `node scripts/e2e-parallel.mjs` and record the suite's time and result beside the 29.7 minutes of 2026-09-11. Deliberately not run on 2026-09-14: the next full run on `main` serves this task **Measured 2026-09-15** (spec 046 T109; `--shards=2` at 9a90baa, `main` 0dbe476 plus spec 046's closing docs and accessibility pass): **52.6 minutes, 449 passed, 1 failed, 1 skipped**, against 29.7 minutes and 398 passed on 2026-09-11, and 58.1 minutes and 437 passed on 2026-09-14. The machine was not busy. The one failure is this spec's own `carried-light.spec.ts:66` (T065): Brom's client stops naming a controlled token partway through his keyboard walk (`named: null`), and a step does not persist. It fails on its own every time at 9a90baa (4 of 4) and at 633825c, before any spec 046 code (2 of 2). T065's proof no longer holds on this machine, and this is left open for the owner.
...
- [X] T065 [US6] A game system's carried light reaches no board. FR-061 and FR-064 require a system to set the bright and dim reach of a light a character carries; D&D 5e declares both (`packs/systems/dnd5e/system.json`, `vision.carriedLight`), `vision_from` resolves them and `tokenVision` returns `carriedBright`/`carriedDim` in world units (`src/server/src/graphql/queries/token_vision.rs:156-157`), and the web reads both and dispatches only `darkvision` (`apps/web/src/engine/world/sync/tokenVision.ts:69-77`). The engine's profile carries darkvision and nothing else (`crates/thunderforge-canvas-core/src/vision_declaration_tests.rs:171`). Needs a decision on what a carried light *is* in the engine — a light attached to the token, as the crawl's lantern is placed by a Game Master, or part of the token's profile — then a playtest step that watches it light a dark board **Decided and built 2026-09-14** (spec decision 6: a carried light is a light attached to its token). The web sends `set_carried_light { tokenId, bright, dim }` beside `set_token_vision`, zeros included (`apps/web/src/engine/world/sync/tokenVision.ts`); the engine keeps it in `LightSet` as `carried:<tokenId>` with `attached_token_id` set, the mechanism a Game Master's token-attached light already used, so it gets its own shadow-map row, lights every seat and follows its token (`src/engine/src/app.rs` `SetCarriedLight`, `LightSource::carried` in canvas-core). `remove_token` removes it; the Game Master's light tools leave it alone. **Evidence:** `e2e/carried-light.spec.ts` passes (3 of 4 runs; the other stalled at Brom's fifth keyboard step, before the torch was in question, and has not recurred): the torch reaches Aria's engine at 200/400 on Brom's token, lights the rat for her, the wall keeps the goblin dark by hidden tokens and by pixels, the light follows Brom's eight-step keyboard walk on her board and the old spot goes dark, and it goes out when the sheet drops it and when the token is deleted. `pnpm playtest --only=dungeon-crawl` passes both systems, with a hard dnd5e step where Brom's sheet torch lights a bat on Aria's board (Genie declares no carried light). Not exercised: a drag moving a carried light (the keyboard walk and remote moves go through the same token transform). *2026-09-15:* the spec now fails every time, alone, at Brom's keyboard walk, including at 633825c before spec 046. See T063.
  - *Reopened 2026-09-15 (owner):* the light is built, but its proof fails every time: `carried-light.spec.ts:66` ("Brom's step 4 west persists") loses Brom's controlled token mid keyboard walk (`named: null`), 4/4 on d75c8f2 and 2/2 on 633825c. Reopened until that is fixed.
  - *Closed 2026-09-15:* the light was never the fault; a player's keyboard walk was, and it could be lost in real play. **Two races, both in the web.** (1) The world store's `upsert_token` *replaced* a token, and the engine's own move events carry only the transform, so every keyboard step or drag wiped the token's owner from the store. `WorldPage` then found no token of the player's, and named `null` to the engine as both viewer and controlled token until the server's event-14 re-read put the owner back; a key pressed in that window moved nothing (`named: null`). Fixed by merging an upsert into the token as it stands (`apps/web/src/engine/world/store.ts`); a field the command carries still wins. (2) With that fixed, the walk failed 2 runs in 5 differently: the re-read triggered by one step could arrive after the next step had left, put the token back a cell, and the next press was measured from there, so the server accepted a move onto a square already stood on. Fixed by `apps/web/src/engine/world/sync/localMoves.ts`: a re-read sent while this client's move of a token was unanswered, or answered after the read left, keeps that token's position and takes the rest (`sync/tokens.ts` `freshestToken`); a refused move is settled before its read-back, so a refusal still puts the token back. Unit tests: `engine/world/__tests__/store.test.ts`, `sync/__tests__/localMoves.test.ts`. **Evidence:** before, `carried-light` failed alone at step 3 with `named: null`; after both fixes it passed 6 of 6 alone (64-66 s each), then `--only=token-keyboard-move,token-authoring,token-movement-walls,darkvision-range,scene-lighting,combat-reach,combat-economy,carried-light` 20 passed, no `✘` (629 s), and `ENGINE_PROFILE=dev pnpm playtest --only=dungeon-crawl` (whose first step walks Aria by keyboard) 2 passed, no `✘` (196 s).
...
- [X] T067 [US6] FR-061's other half: a Game Master sets a placed light's bright reach apart from its dim reach (owner, 2026-09-15). A stored light had one `radius`, drawn bright to half of it. **Built 2026-09-15.** Migration `src/server/migrations/2026-09-15-140000-0000_light_bright_radius/` adds `light_sources.bright_radius` (NOT NULL, `CHECK (bright_radius >= 0 AND bright_radius <= radius)`), filled with `radius * 0.5` so every existing light draws as it did (FR-062); `radius` stays the dim reach. `down.sql` drops the column and the check. GraphQL: `brightRadius` on `LightSource` and on both light inputs (omitted on create, half the radius; on update, a bright reach the new dim reach would fall inside of is brought in to meet it; naming both with bright past dim is refused), and `sceneUnits(sceneId) { perCell label gridSize }` so chrome can speak the system's units (`graphql/queries/token_vision.rs`). The dd2vtt import and scene copy carry the bright reach; the server's own sight (`combat/redaction.rs`) reads it. Engine: `upsert_light` takes `brightRadius`; the Game Master's wheel resizes the dim reach and Shift+wheel the bright one, undo restores both, a placed light is created bright to half its reach, the selected light's two reaches are drawn as rings, and the engine now tells chrome which light is selected (`select_light`), without which the panel's fields could not be reached (`systems/lighting_edit.rs`, split from `lighting.rs`). The Lights panel's Radius field became **Bright reach** and **Dim reach** in the system's units, committed on Enter or blur and refused in place when bright passes dim (`LightingTool/LightReachFields.tsx`). **Evidence:** migration up, down and up again on a scratch database (two one-radius lights of 400 and 75 became bright 200 and 37.5; a bright reach past the dim one and an insert naming none are both refused; down leaves the radii). `e2e/light-reach.spec.ts` passes through the harness: a one-radius light reaches the Game Master's and two players' engines bright to half; the Game Master types 40 ft dim and 20 ft bright into the panel; the server stores 400/200; all three engines light by 200/400 (`__engineProbe.placedLights()`); on Aria's board 15 ft out is brighter than 30 ft out, which is brighter than 55 ft out; after a reload the panel reads 20/40 and refuses 50 ft bright. Re-run beside it, all passing: `scene-lighting`, `carried-light`, `darkvision-range`, `token-authoring`, `combat-attack`, `combat-hit-points`, `token-links`.
