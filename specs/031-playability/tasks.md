...
- [X] T015 **Note, 2026-09-10 (playtest P2)** — the marker came back. R6's browser check of where it lands was never recorded as done, and nothing linked its colour to the light marker although the 2026-09-01 report named the light tool. See `playtest-2026-09-10.md`. [US9] Confirm the stray-marker cause before fixing — check whether the marker lands at the tool *button's* screen position rather than the pointer's last map position (research R6), and record the finding in `specs/031-playability/research.md`
- [X] T016 **Note, 2026-09-10 (playtest P2)** — entering a mode is inert, as intended; the defect is that a mode outlives the rail. Closing the rail leaves the engine armed with the last tool (`WorldPage.tsx`, deliberately), so after Lights every map click places a light. The fix returns the engine to Select when the rail closes. [US9] Make entering an authoring mode inert on top of T008 so no tool switch places content, in `src/engine/src/plugins/` (FR-040, FR-040a)
...
- [X] T032 **Reopened and closed 2026-09-10 (playtest P1)** — the begin half was wired and the confirm half was not: the engine's `token_placement_confirmed` for `kind: "actor"` had no listener (`InteractionTool.tsx`'s only listener returns for anything but `"prop"`), so Place created no token. Commit eb8b1ad had wired props only. `pages/world/usePlaceActorTokens.ts` now answers an actor's drop with `createToken`; e2e `canvas-authoring.spec.ts` › "Place creates a token, and the token shows the character's own art". See `playtest-2026-09-10.md`. [US1] Wire Place to begin an engine placement and, on confirmation, call the token-create mutation subject to existing ownership rules, in `apps/web/src/` (FR-004, FR-007, contracts/graphql-mutations.md)
