...
- [X] T002 Run `diesel migration run` against the local dev DB and regenerate `src/server/src/schema.rs` with the new table (depends on T001)
- [X] T003 [P] Add `ContentModerationAction` Diesel `Queryable`/`Insertable` struct to `src/server/src/models.rs`, matching the existing struct-per-table convention (depends on T002)
...
- [X] T005 [P] Create `src/server/src/moderation/validation.rs`: `validate_takedown_notice(input) -> Result<(), Vec<MissingElement>>` and `validate_counter_notice(input) -> Result<(), Vec<MissingElement>>` per data-model.md's validation rules (FR-003/FR-006) — pure functions, no DB access, easy to unit test in isolation (depends on T003)
- [X] T006 Add `MODERATION_COUNTER_NOTICE_WAITING_PERIOD_DAYS` and `MODERATION_REPEAT_INFRINGER_LOOKBACK_DAYS`/`MODERATION_REPEAT_INFRINGER_THRESHOLD` as config values (env-var-backed, with the 10-14 business day default noted in research.md R3) in `src/server/src/config/mod.rs` (or wherever existing config values of this shape live), NOT hardcoded literals in `moderation/mod.rs` (depends on T004)
- [X] T007 [P] Add `ModerationEntityType`, `ModerationActionType`, `GraphQLModerationAction`, `GraphQLModerationCase` GraphQL types to `src/server/src/graphql/types.rs`, matching the existing enum/SimpleObject conventions (contracts/graphql-moderation.md) (depends on T003)
- [X] T008 [P] Add `SubmitTakedownNoticeInput`, `SubmitCounterNoticeInput` GraphQL input types to `src/server/src/graphql/input_types.rs` (contracts/graphql-moderation.md) (depends on T003)
- [X] T009 Register the `moderation` module in `src/server/src/main.rs` (or wherever top-level modules are declared) (depends on T004, T005)
- [X] T010 [P] Add `ModerationCaseRecord`, `ModerationActionRecord` TypeScript types to `apps/web/src/types/moderation.ts` (contracts/graphql-moderation.md)
- [X] T011 [P] Create `apps/web/src/api/moderation.ts` with `fetch`-based GraphQL call stubs mirroring `apps/web/src/api/items.ts`'s `postGraphQL`/CSRF pattern (depends on T010)
...
- [X] T042 **Done 2026-09-13** — closed before spec 050's sync-back ships, as the owner required (ADR-098). *Originally recorded by spec 039 T093:* a scene was not a moderated entity type (`collections::moderation_entity_type` returned `None` for `"scene"`), so a scene in a shared collection was never withheld from the collection's read and copy paths, no notice could name a scene, and an adopted scene recorded no adoption for a takedown to follow. **What now holds:** `ModerationEntityType::Scene` (db string `scene`) is filed through `/legal/dmca` like the other four; the owner is `scenes.owner_id`, so the strike, the owner's notice and the repeat-infringer count all follow. `moderation_entity_type("scene")` is `Some("scene")`, so `collections::resolve::resolve_member` withholds a taken-down scene from `shared_collection_impl` and from `copy_shared_collection_to_world_impl`, asked through `moderation::effective_status` on every read; a test now fails if any member type lacks a moderation entity. `copy.rs::record_adoptions` records scene adoptions, and `moderation::reach::name_of` names a copied scene — without that arm the walk would have read the copy as deleted and skipped it. In its own world a taken-down scene is absent for every caller including its owner and a site admin: `scenes`, `scene`, `walls`, `lightSources`, `shapes`, `tokens`, `fogMask`, `auth::scene_visibility` (so the sync plan and `GET /canvas-assets/{id}` for any asset attached to the scene) and the scene preview route. `effective_status_sync` is the same rule on a held connection, and the async form now wraps it. `attestation::CoveredKind::Scene` lets a notice handler find the collection agreement behind a scene. The web takedown form offers "Scene", and the standing page offers the counter-notice for any strike whose content has no page of its own — a scene has no detail page for `ModeratedContentBanner`, so before this its owner had no route back unless their account was already disabled. **Proof:** `moderation/scene_tests.rs` (notice disables for everyone and only that scene; counter-notice restores after the waiting period and removes the strike; three scene takedowns flag the account), `collections/copy_scene_moderation_tests.rs` (withheld from link and copy, unnamed, returned by counter-notice; a scene copied before its takedown goes dark as a copy, is nobody else's strike, and returns with its source), `collections::tests` (every member type maps to a notice-nameable entity); `e2e/dmca-scene-takedown.spec.ts` drives the notice logged out, the signed-out link losing and regaining the scene, and the owner's counter-notice from the standing page (8/8 green with the other takedown, counter-notice and collection-moderation specs); `cargo test -p thunderforge-server` 1501 passed. **Not covered, said plainly:** (1) a scene's images were **not** separately moderated before this — no `ModerationEntityType` has ever named a canvas image, so this task's old wording ("filed against its images") and spec 038 FR-026c's parenthesis describe a mechanism that did not exist. Images are now withheld only *through* their scene; an image that belongs to no scene, a legacy `background_image_path` served from the static `/assets/` route, and the same stored object reused by an unrelated upload (dedupe by content hash) are not reached — ADR-079 rejected content-hash reach for good reasons. (2) Interactives, the world event stream and subscriptions are not gated on a taken-down scene; they carry no scene art or scenery, but a live table playing the scene at the moment of takedown keeps its already-loaded state until it next loads. (3) The user's own data export still includes the scene, as it does the other moderated types. (4) ADR-098 condition 3 (a takedown withholds every **version**) is not testable yet — collection versions do not exist; when spec 050 adds them, the version read must go through `resolve_member` or `effective_status` too.
  - *Live tables, 2026-09-13:* gap (2) above, a table already playing a scene when it is taken down, is not closed here and no longer tracked here. It is reached by [spec 051](../051-operator-playfield-shutdown/spec.md): a takedown on content in live play raises a request to pause that world, and an operator's pause ends its live play (051 FR-030, ADR-100). A takedown still withholds only what it withheld before.
