...
- [X] T073 Run the full suite via `node scripts/e2e-parallel.mjs --shards=2` and record the figures in the commit body — **362 passed / 2 failed in 40.3 min** (2026-09-09). Both failures real, both diagnosed and fixed: a sign-in race in this spec's own FR-008 test, and `github-apps.spec.ts` asserting the opposite of the credential-fallback feature because its premise (a configured sync application) was never established on a harness that clears `SYNC_GITHUB_APP_*`
- [~] T074 Run `pnpm verify` and fix what it reports **in the code this feature added** — keep it to that; wide lint passes get their own commit. `pnpm verify:fix` rewrites what is mechanical — **partially done**. It found and I fixed a real `clippy::let_and_return` in `subscriptions.rs`, in the FR-010 code added today, which neither `cargo check` nor the commit hook flags. Light checks all pass: web format (against the project's own glob, which excludes `e2e` deliberately), web lint, file length, SDK bindings. **The Rust lint could not be completed**: the machine's OOM guard killed nine consecutive cargo invocations, down to `cargo check --lib -j 1`. The fix itself is textually the change clippy's own `help` suggested — a `match` returned directly instead of bound and returned — but it has not been recompiled. Re-run `make lint-host` on a quiet machine before trusting it
