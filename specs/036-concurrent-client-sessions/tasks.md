...
- [X] T074 Run `pnpm verify` and fix what it reports **in the code this feature added** — keep it to that; wide lint passes get their own commit. `pnpm verify:fix` rewrites what is mechanical — **partially done**. It found and I fixed a real `clippy::let_and_return` in `subscriptions.rs`, in the FR-010 code added today, which neither `cargo check` nor the commit hook flags. Light checks all pass: web format (against the project's own glob, which excludes `e2e` deliberately), web lint, file length, SDK bindings. The Rust lint took nine attempts to run at all — see the note below on why — and then passed: `cargo clippy -p thunderforge-server --lib` **exit 0**, with the `let_and_return` fix in place
