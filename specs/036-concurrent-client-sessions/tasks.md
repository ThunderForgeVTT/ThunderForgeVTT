...
- [ ] T032 [P] [US4] Add server tests in `src/server/src/graphql/mutations_sessions.rs` for `mySessions`, `endSession` (own only; another account's id is indistinguishable from a missing one) and `endAllSessions`
...
- [ ] T034 [US4] Implement the session registry reads in `src/server/src/auth/session_registry.rs` (live sessions, last seen, current, holds-play-field)
- [ ] T035 [US4] Implement `mySessions`, `endSession` and `endAllSessions` in `src/server/src/graphql/mutations_sessions.rs` per `contracts/sessions.md`, writing `ended_reason`
...
- [ ] T039 [US4] Extend `apps/web/e2e/concurrent-sessions.spec.ts` with US4's five acceptance scenarios
