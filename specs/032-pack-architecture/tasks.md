...
- [ ] **T108 [US2] A pack declares the panels it contributes, the way it now declares its sheet.** Widening `check-system-registry.mjs` to shared *web* code (2026-09-03) found four violations that had been invisible because nothing was looking: `ActorDetailPage.tsx`, `WorldStagingPage.tsx`, `WorldSystemSettingsPage.tsx` and `ClocksPanel.tsx` each branch on one system's id to mount that system's panel from a page every system shares. They are on the check's `KNOWN` list, which is where a violation waits with a task id rather than where it goes to be forgotten.
  They are listed rather than fixed alongside the actor sheet because they need something the sheet did not — and the first version of this entry said what that was and got it wrong. It claimed a filename is the wrong place to encode which slot a panel fills; `packs/systems/<id>/web/src/panels/<slot>.tsx` carries both fine, and one glob reads them off the path. The actual difference is that panels need a **host-declared vocabulary**: an agreed set of slot names and a typed props contract for each, because a pack has to know which slots exist and what each is handed. The sheet needed exactly one contract, so a filename convention was the whole of it. That vocabulary is the work here. See ADR-066.
