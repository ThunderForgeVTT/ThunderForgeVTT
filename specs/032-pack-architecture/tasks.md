...
- [ ] **T108 [US2] A pack declares the panels it contributes, the way it now declares its sheet.** Widening `check-system-registry.mjs` to shared *web* code (2026-09-03) found four violations that had been invisible because nothing was looking: `ActorDetailPage.tsx`, `WorldStagingPage.tsx`, `WorldSystemSettingsPage.tsx` and `ClocksPanel.tsx` each branch on one system's id to mount that system's panel from a page every system shares. They are on the check's `KNOWN` list, which is where a violation waits with a task id rather than where it goes to be forgotten.
  They are listed rather than fixed alongside the actor sheet because they need something the sheet did not. The sheet had exactly one mount point, so a filename convention was enough — `packs/systems/<id>/web/src/ActorSheet.tsx`, found by a build-time glob. A panel has four different mount points and will have more, so the convention would have to encode *where*, and a filename is the wrong place to put that. What it wants is a declaration: a pack naming the slots it fills, discovered the same way, which is the web analogue of the world-creation hook that retired the server's last entry. See ADR-066.
