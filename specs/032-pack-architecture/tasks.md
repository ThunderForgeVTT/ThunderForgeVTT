...
**T108 is no longer deferred.** It is kept here, done and struck through in
place, because the reasoning that deferred it — and the correction to that
reasoning — is the useful part of the record, and moving the entry to a
completed section would leave a reader wondering why panels waited when the
sheet did not.

- [X] **T108 [US2] A pack declares the panels it contributes, the way it now declares its sheet.** Widening `check-system-registry.mjs` to shared *web* code (2026-09-03) found four violations that had been invisible because nothing was looking: `ActorDetailPage.tsx`, `WorldStagingPage.tsx`, `WorldSystemSettingsPage.tsx` and `ClocksPanel.tsx` each branch on one system's id to mount that system's panel from a page every system shares. Done 2026-09-04; the check's `KNOWN` list is empty on both halves for the first time.
  They were listed rather than fixed alongside the actor sheet because they needed something the sheet did not — and the first version of this entry said what that was and got it wrong. It claimed a filename is the wrong place to encode which slot a panel fills; `packs/systems/<id>/web/src/panels/<slot>.tsx` carries both fine, and one glob reads them off the path. The actual difference is that panels need a **host-declared vocabulary**: an agreed set of slot names and a typed props contract for each, because a pack has to know which slots exist and what each is handed. The sheet needed exactly one contract, so a filename convention was the whole of it. See ADR-066.
  What shipped: `PanelSlot` and `PanelSlotProps` in `@thunderforge/host` (`npc-detail`, `world-staging`, `world-settings`, `clocks`); `apps/web/src/panels/systemPanels.ts`, an `import.meta.glob` registry keyed `` `${systemId}:${slot}` `` and shaped exactly like `systemActorSheets.ts`; four slot files in `packs/systems/genie/web/src/panels/`, two of which point at one component because the session loop is one panel reached from two places.
  **Three things fell out of it that were larger than the entry.**
  *The panels could not move without their data layer.* A pack imports from `@thunderforge/host` or from nowhere, so a pack panel calling `useGenieSession` from `apps/web/src/hooks` was not an option — and exporting a system-named hook from the host would have been the violation with extra steps. `api/genieSession.ts`, `hooks/useGenieSession.ts` and `engine/world/sync/genieSession.ts` moved into `packs/systems/genie/web/src/session/`, and the host gained `postGraphQL` and `subscribeToWorldEvents` to make that possible. That is a real widening, and the honest one: ADR-063 already gave the pack the tables and the GraphQL over them, and a pack owning a schema it may not call is not a boundary — it is a pack whose halves have to meet in `apps/web`, which is where every violation this app has had came from.
  *The filename check found five more files, not one.* The entry expected `GenieShopPanel.tsx` and `GenieSessionPanel/`; the other three were the data layer above, none of which quoted the id inside itself. `check-system-registry.mjs` now compares each shared file's path against every bundled id with separators flattened, so `YearZeroEnginePanel.tsx` trips on `year_zero_engine`.
  *Pack web code was type-checked by nobody.* The pack's own `tsc --noEmit` reported "Cannot find module '@thunderforge/host'" on every file that used it — including `ActorSheet.tsx`, since ADR-066 landed. Mapping the alias inside the pack pulls `apps/web` under the pack's React 18 types and fails on the version skew; `apps/web/tsconfig.json` now `include`s `../../packs/systems/*/web/src` instead, which checks pack code against the React the bundle actually uses. A typed slot contract nothing checks is not a contract.
  Mutation-tested five ways, all five caught: break the glob (empty registry), quote a system id in shared web code (content check), add a `GenieThing.tsx` under `apps/web/src` (filename check), add a `YearZeroEnginePanel.tsx` (flattened-separator filename check), fork `clocks.tsx` instead of re-exporting (the shared-component assertion).
  **Left behind on purpose:** `worlds.genie_resource_carryover_enabled`, still a column named for one ruleset on a shared table, and still invisible to this check because a column name is neither a quoted id nor a filename. Slotting the panel does not fix it; a migration does.
