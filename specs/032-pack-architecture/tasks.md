...
- [ ] T014a Two pre-existing violations the new checker found, currently in its dated `KNOWN` list rather than papered over: `src/server/src/graphql/helpers.rs` defaults a new world's `game_system_id` to `"genie"`, and `src/server/src/graphql.rs` branches on `is_genie_world` during world creation. Both are shared code deciding something per system — SC-004's violation, pre-dating this feature and behavioural, so they were recorded rather than changed mid-increment. Fixing them empties `KNOWN`, and the checker fails if an entry there stops being a violation and is left behind
