...
> - **"Uploaded" is scoped to what the PDF content readers produce**: source books and character sheets. Imported maps and uploaded images count as authored and are policed by takedown instead. **Decided by the owner 2026-09-13**, and ADR-097 reworded, because its literal first wording would have made a scene on an imported map unshareable.
...
> - **The first cut contradicted spec 050 decision 5.** Switching a book off cascaded every delta away, *additions included*, so a Game Master's authored homebrew was destroyed, although decision 5 says additions stay. The agent had flagged it as open because my brief omitted decision 5. **Fixed in `b29180d`**: a generated column exempts additions from the book-list foreign key. Kept additions show under "Written at this table", switching the book back on finds the same row, and the switch-off report lists what is kept. The same reasoning then showed FR-061 was wrong too, since removing a book from the shelf still deleted additions. **FR-061 is amended and that fix is in progress.**
> - **`world_books.switched_on_by` blocked account deletion**: `NOT NULL` with no `ON DELETE`, so a Game Master or Trusted Player who switched a book on in someone else's world could not delete their account. This came from phase 9. **Fixed in `b29180d`** with `ON DELETE SET NULL`, and tested by deleting a Trusted Player's account: the book stays switched on.
> - **Deltas now reach the origin guard**: `content_origin` has a `world_entry_delta` branch. No route can put a delta into a collection yet, but raw SQL proves the guard refuses changed and hidden deltas and accepts added ones.
...
- [X] T094 [050-US6] **Re-confirmed 2026-09-13, with a condition attached by the owner.** Spec 015 is 41/42 done and its takedown and counter-notice e2e are in the suite. Its one open task, **spec 015 T042 — scenes are not a moderated entity type**, means a takedown cannot reach a scene inside a collection; the owner requires **T042 closed before sync-back ships**. **(a)** Re-confirm the notice-and-takedown programme is operational — it was confirmed for ADR-069 and extended by ADR-079. Re-confirming, not re-establishing
- [X] T095 [050-US6] **Accepted by the owner, 2026-09-13: not a repository, on three conditions** (versions owner-only; sync-back never reaches copies; a takedown covers every version). Phase 15 still waits on spec 015 T042. **(b)** `docs/adrs/20260912-098-an_update_path_to_a_collection.md`, **amending ADR-069** and **signed by the accountable owner**. ADR-069's own limits name versioned collections and update paths to already-copied content as re-opening it, and 050 FR-104 is such a path. Research §9 states the question it must answer: spec 026 says an adopted copy is independent, so a sync-back plausibly does not reach copies others took — a reasonable reading, and not the implementer's to assert
- [X] T096 [050-US6] Sync a world's change back to the **collection** it came from (050 FR-100), creating a new version of its base and leaving the previous one recoverable (050 FR-104)

      **Gate status 2026-09-13: both gates are clear.** ADR-098 was accepted with conditions, and spec 015 T042 closed in `8ce0753`. Two things T042's work established that this phase must honour:
      - **ADR-098 condition 3** (a takedown withholds every version) cannot be tested until versions exist. The version read MUST go through `collections::resolve_member` / `moderation::effective_status`, like the live read in `shared_collection_impl`, or the condition fails silently.
      - A table already playing a scene when a notice arrives keeps what it loaded until the next load, because interactives, the event stream and subscriptions are not gated. Syncing a collection back must not become a way to refresh withheld content into a live table.
- [X] T097 [050-US6] **No route at all** from a world to an imported compendium's base, at any volume of change (050 FR-101), and the reason stated where the absence would be noticed (050 FR-102)
- [X] T098 [P] [050-US6] Syncing back shows what will change on the shelf and requires explicit confirmation (050 FR-103) — it writes to something every other world is reading
- [X] T099 [050-US6] A world that has synced no longer holds the change as a delta (050 FR-105); other worlds' deltas behave exactly as after a re-import
- [X] T100 [050-US6] Extend `apps/web/e2e/library-deltas.spec.ts` — a synced change reaches a world started afterwards; the syncing world no longer holds it; and **no route exists** from a world to an imported base, attempted through every surface that offers the sync for collections (050 FR-089, FR-089a)
- [X] T101 [050-US6] Proved: that e2e green; ADR-098 ACCEPTED and owner-signed; `pnpm verify` green

> **Phase 15 notes (verified 2026-09-16).** Sync-back is for collections on the owner's shelf, and a book read in has no route to its base at all.
> - **Versions.** `shelf_collection_versions` keeps the version every change replaces: a write, a removal, a sync, a restore. A restore is a new version ("Went back to version N"), so it can be regretted too. Owner-only, from the library's "Earlier versions" panel. A database trigger refuses a version row for any compendium that is not `Authored`, so no route through the code, or around it, writes history for an imported book.
> - **ADR-098 condition 3 is pinned, not remembered.** The owner chose to build now and record how it reads: conditions 1 and 2 hold by construction (a shelf collection has no share link and no adoption) and condition 3 holds vacuously (no notice can name a shelf entry, current version included). Every read of an earlier version goes through `versions::read_at`, and `an_earlier_version_is_read_in_one_function` fails if a second one appears. The ADR's dated note says the rest: the version *in force* is also read by `collections::download` and `book_list::entries_served_by`, so a future moderation check goes into those too.
> - **The sync.** `worldSyncBackPlan` names every change before and after, and what does not attach and so stays; `syncBackToCollection` lands only with that plan's stamp, and refuses if the collection or the deltas moved on. It records the version, applies the changes, deletes exactly the synced deltas, and reports other worlds' stranded changes as a re-read does. World owner only (a GM or Trusted Player is told why); gated by play pause.
> - **Where the absence is noticed.** A browsed book and a switch-off report with changes both say, in place of the control, that a book's entries record what the book says and nothing is written back. The switch-off report offers the sync first for a collection.
> - **Observed**: Rust lib tests for deltas, compendium, library deletion and the pause surface 123 passed (the pause test needs `RUST_MIN_STACK`, as `make test` sets); app crate pause surface 3 passed; e2e `library-deltas` (new sync-back journey included), `library-collections`, `library-account-deletion` 7/7; `tsc --noEmit` clean. `pnpm verify` 14/14 green once the GraphQL schema snapshot was regenerated. `make lint-host` is red on `thunderforge-pdf` and `pack_system_spec`, neither touched here; clippy reports nothing in this phase's files.
