...
- [ ] T001 Record the baseline: run `cargo run -p dnd5e-server --example harvest -- <corpus bestiaries>` and keep the report. It currently yields **717 creatures and 747 per-attack reach values**; every later phase is measured against that, and a drop is a regression rather than a rounding difference
- [ ] T002 [P] Read `specs/049-importing-a-source-book/contracts/content-patterns.md` and `contracts/import.md` into the working set — they are what every phase below is measured against
- [ ] T003 [P] Confirm the corpus is reachable and `cargo run -p thunderforge-pdf --example survey -- <corpus-dir>` still reports 246 opened / 196 with text, so Phase 4's measurement has a baseline to move from
...
- [ ] T005 [P] Validation in `validate_system_manifest` in the same file: reject two patterns sharing an `anchor`, reject a duplicate `kind`, and reject a `prose` pattern that declares `fields` — a silently ignored field list is a promise somebody thinks they have (contract: content-patterns, "rules a pack must satisfy")
- [ ] T006 [P] `crates/thunderforge-canvas-core/src/content_patterns.rs` — the runtime declaration, mirroring `vision_declaration.rs`: `ContentPatterns`, `Pattern { kind, shape, anchor, name_rule, fields }`, `Shape::{Anchored, Prose}`, `NameRule`, `FieldSpec { key, label, kind }`, plus `Default` returning an empty set
- [ ] T007 `src/server/src/content_patterns.rs` — the loader, mirroring `vision_profiles.rs::vision_declaration_for_system`: read `<systems_dir>/<system_id>/system.json`, deserialize the `contentPatterns` key only, and treat missing/unreadable/malformed as absent rather than as an error
- [ ] T008 [P] The 5e declaration in `packs/systems/dnd5e/system.json`: five kinds — `creature` anchored on `Armor Class`, `spell` anchored on `Casting Time`, `magicItem`, `classFeature` and `feat` as prose — with the creature `name` rule expressing "walk back up to six lines and take the largest", which is the test this design had to pass (049 FR-013)
- [ ] T009 [P] Unit tests in `crates/pack_system_spec` covering each validation rule, and a round-trip of the real 5e block so the schema and the shipped manifest cannot drift apart
- [ ] T010 [P] `docs/adrs/20260912-096-content_patterns_as_a_manifest_extension_point.md`, PROPOSED: an extension point in the shape `vision` already established, why the schema crate duplicates rather than imports, and why the declaration lives in the manifest rather than in a pack's Rust (it has to be readable by the browser, which is where the reading happens)
- [ ] T011 Proved: `cargo test -p pack_system_spec` green, `node scripts/check-system-registry.mjs` passes **with `KNOWN` still empty**, and a system with no `contentPatterns` yields a refusal with a reason rather than an empty success
...
- [ ] T012 [049-US5] `src/server/src/content/anchored.rs` — takes `thunderforge_pdf::layout::Line`s and a `Pattern`, finds each anchor, reads the declared fields, and applies the `NameRule`. It contains no game vocabulary: every label it looks for arrives from the declaration
- [ ] T013 [P] [049-US1] `ReadValue { state: Read, value: Option<String> }` in `src/server/src/content/mod.rs`, generalising `statblock.rs`'s `Confidence` — `read` / `uncertain` / `unread`, and **`unread` carries no value** (049 FR-002, data-model "read value"). A value that will not parse as its declared type is `uncertain` with the text as read, never coerced and never dropped
- [ ] T014 [049-US5] Extend `SystemContribution` in `crates/thunderforge-canvas-core/src/system_contribution.rs` with an optional per-kind **refinement** `fn` pointer: it receives the generic entry plus the lines it came from and returns a refined entry. This is the mechanism for what a declaration cannot express, and it is the one that already exists for pack-owned behaviour
- [ ] T015 [049-US5] Per-attack reach moves to a 5e refinement in `packs/systems/dnd5e/server/src/`, submitted through the existing `inventory::submit!` in that pack's `lib.rs`. Spec 045's playtest established reach is **per attack**, not per creature size, so it cannot be dropped — and it is not a labelled field, so it cannot be declared (contract: content-patterns, "what the declaration deliberately cannot express")
- [ ] T016 [049-US5] Retire `statblock.rs`'s general machinery in favour of the generic reader, keeping only what became the refinement. Safe to do: `statblocks()` has **no production caller** — only `examples/harvest.rs` and its own tests — so nothing regresses
- [ ] T017 [P] [049-US5] Point `packs/systems/dnd5e/server/examples/harvest.rs` at the generic reader so the baseline from T001 is measured on the new path
- [ ] T018 [P] [049-US1] Unit tests for `anchored.rs` in `src/server/src/content/anchored_tests.rs` — a field present, a field absent (unread, no value), a field that will not parse (uncertain, text kept), two entries in sequence, and an anchor appearing inside another entry's prose
- [ ] T019 [049-US5] Proved: `harvest` on the same bestiaries reports **≥717 creatures and ≥747 reaches**, `cargo test -p thunderforge-server -p dnd5e-server` green, and `node scripts/check-system-registry.mjs` still passes with `KNOWN` empty
...
- [ ] T020 [049-US1] `src/server/src/content/prose.rs` — finds entries by the declared `name` rule (bold or heading) and runs each to the next one. No fields, and the type gives it nowhere to put one (049 FR-001b, data-model "entry")
- [ ] T021 [P] [049-US1] Tests in `src/server/src/content/prose_tests.rs` including the boundary case that matters: a bold run-in name *inside* a paragraph must not start a new entry
- [ ] T022 [049-US1] Propagate `thunderforge_pdf`'s per-line `suspect` flag onto any entry built from those lines (049 FR-004), so the review can show which entries came from pages the reader struggled with
- [ ] T023 [049-US1] Extend `crates/thunderforge-pdf/examples/survey.rs` to report, per book: opened, silent pages, and **entries found per kind** — reusing the existing walk rather than adding a second tool that would disagree with it
- [ ] T024 [049-US1] Hand-count spells in three corpus books and compare, to answer 049 SC-002's 90% target — which the spec already records as a target rather than an observation
- [ ] T025 [049-US1] **Proved, and the readers are now evidence-backed**: `specs/049-importing-a-source-book/measurements.md` in the shape spec 043's took — corpus provenance, verdict rule, results, threats to validity, what would reopen it — with numbers in generated JSON carrying `generatedAt`/`generatedBy`. If SC-002 is not met, **correct the spec** rather than leaving it aspirational
...
- [ ] T035 [049-US2] `createCompendiumFromImport` in `src/server/src/graphql/mutations_compendium.rs`, with the input from `contracts/import.md`. **Origin is not an input** — the server writes it (049 FR-051)
- [ ] T036 [049-US2] Server re-checks on arrival, all four, before anything is written: the caller is the owning account, the system exists and declares patterns (re-read from the manifest, not trusted from the payload), the entry count is within bound, and every entry's kind is one the system declared. A review in a browser is not a permission (049 FR-036, Principle III)
- [ ] T037 [049-US2] Apply inside `conn.transaction(...)`, following `src/server/src/map_import/mod.rs:230` — one row at a time inside one transaction, which is what every existing bulk write in this repository does
- [ ] T038 [P] [049-US2] The stated entry bound (049 FR-035), refused in the browser before sending **and** on the server. It is what keeps the transaction sane, so it belongs in the contract rather than being discovered at the database
- [ ] T039 [049-US2] `apps/web/src/api/compendium.ts` — submit with progress that reflects bytes actually sent and says *what* is being sent (049 FR-030, FR-031). Honest about what the bar means: reaching the end is "the server has it and is applying it", not "applied"
- [ ] T040 [P] [049-US2] Abandonment (049 FR-034): dropping the request leaves nothing, because nothing partial was ever committed
- [ ] T041 [049-US2] `apps/web/e2e/book-import-commit.spec.ts` — induce a failure partway and assert the account is byte-identical to before (049 SC-007)
- [ ] T042 [049-US2] Proved: that e2e green; `cargo test -p thunderforge-server` green; `tsc --noEmit` clean
...
- [ ] T043 [050-US1] Migration in `src/server/migrations/2026-09-12-HHMMSS-0000_compendium/` — `compendiums` and `compendium_entries`, **owned by an account** (049 FR-040), with `created_by`/`updated_by` per the convention `world_collections` sets. Never world-owned "for now"; research §7
- [ ] T044 [050-US1] `src/server/src/auth/account_ownership.rs` — `require_account_owner`, fail-closed, beside `require_world_member` in `auth/world_membership.rs`. **Net-new: no account-scope helper exists** and this arc asks the question in six places (research §11)
- [ ] T045 [049-US3] `src/server/src/compendium/` — the store, with origin written on insert and **no update path to it at all** (049 FR-057). A value that can be flipped is a value that will be flipped
- [ ] T046 [P] [049-US3] `src/server/src/graphql/queries/compendium.rs` — `myLibrary`, `compendium`, `compendiumEntries`, every one account-scoped through T044's helper
- [ ] T047 [049-US3] `removeCompendium(id, confirm)` — with `confirm: false` it reports what is in use and what was hand-edited and **changes nothing**; with `confirm: true` it removes that import's contribution and nothing else (049 FR-044, FR-045). Two calls because naming what is in use *before* confirming cannot be done in one
- [ ] T048 [049-US3] Re-import against the account's library (049 FR-047, spec 047 FR-070–FR-075): recognised by hash, overwrite offered, hand-edited entries untouched, and a *near*-identical file **not** claimed as a match
- [ ] T049 [P] [049-US3] `apps/web/src/pages/library/` — the account shelf: each compendium with its book, system, date, counts and origin. A **new account-level surface**, not a change to the world Compendium portal
- [ ] T050 [049-US3] Proved: two books import as two compendiums with their own counts; a re-import is caught from a second world; removal names what is in use first; `cargo test -p thunderforge-server` and `tsc --noEmit` green
