...
- [X] T007 **Landed with Phase 6, 2026-09-10** — the second section of `legal/sharing-terms.md` is the draft in `drafts/sharing-terms-t007.md`, in the same commit as the takedown's reach, so the terms never promised a reach that did not exist nor denied one that did. It minted a new version; earlier agreements keep resolving to the words they were made against. **Still the owner's to revise** — drafted at their request and not yet reviewed by them, nor by a lawyer (`legal/README.md`); a revision is one more version, and costs nobody their agreement. In `legal/sharing-terms.md`, revise the second section: "A copy someone takes is theirs, and **cannot be recalled**" becomes untrue for copies disabled by a takedown the day FR-023 ships. **Reviewer decision** — the product must not tell people something it no longer does
...
- [X] T040 **Done** — `reach_tests.rs`'s `a_copy_taken_down_with_its_source_is_nobodys_strike`: the child case has `account_id = NULL` and its own `case_id` pointing at the notice's, the adopter holds no strike, and the sharer's one strike is unchanged by how many copies there were. [P] [US6] Add a test in `src/server/src/moderation/reach.rs` asserting a child case carries `account_id = NULL`, so an adopter accrues **no strike** for somebody else's upload (FR-023b) — give it the adopter's id and the test must fail
- [X] T041 **Done** — `the_walk_is_transitive_and_passes_through_a_deleted_copy`: a copy of a copy is disabled, and a middle copy its adopter deleted does not hide the one made from it. [P] [US6] Add a test asserting the walk is transitive: a copy of a copy is disabled (data-model.md § 3)
- [X] T042 **Done** — `graphql/adoption_surface_tests.rs`, two halves: a walk of both crates' source that fails if any file but `moderation/reach.rs` (and Diesel's `schema.rs`) names the table, and an SDL check for anything speaking of adoption. The SDL half caught its author's own doc comment ("an adopted copy") on its first run — async-graphql publishes doc comments as descriptions — and "provenance" was dropped from its word list because the schema already uses it honestly elsewhere. [P] [US6] Add an SDL test asserting **no** field, query, subscription or route reads `content_adoptions` — the assertion that keeps ADR-069's determination true
- [X] T043 **Done before this phase, 2026-09-07** — `lore_sync/plan_tests.rs`'s `a_disabled_entry_is_absent_and_does_not_stop_the_others`, with the fixture corrected to `world_lore_entry` and a comment recording that fixture and defect had agreed with each other. [P] [US6] Add a failing-first test in `src/server/src/lore_sync/plan.rs` proving taken-down lore reaches the Git mirror today, because `filter_visible` is called with `"lore_entry"` while every moderation row carries `"world_lore_entry"`

### Implementation for User Story 6

- [X] T044 **Done** as `2026-09-10-130000-0000_content_adoptions` — dated after the migrations already applied rather than 2026-09-07, which would sort before them. The comment states the no-reader rule and the scene gap. `destination_world_id` cascades with the adopter's world; the source and `adopted_by` carry no key, so the walk still passes through a deleted source. [US6] Create the migration `src/server/migrations/2026-09-07-131000-0000_content_adoptions/up.sql` and `down.sql` per data-model.md § 3, with a comment stating the no-reader rule and recording that `"scene"` maps to no moderation entity so scenes record no adoption
- [X] T045 **Done** as `2026-09-10-131000-0000_moderation_parent_case`, with a partial index. The column is on the table but deliberately not on `NewContentModerationAction`: `reach.rs` writes child events itself, so the dozens of existing struct literals are untouched and nothing else can write a child case by accident. [US6] Create the migration `src/server/migrations/2026-09-07-132000-0000_moderation_parent_case/up.sql` and `down.sql` adding nullable `parent_case_id` to `content_moderation_actions`; run `make schema` and extend the models
- [X] T046 **Done** — `copy.rs`'s `record_adoptions`, from the source→copy maps, so dependents count too (an ability copied inside an actor). The module header now says a copy is independent to its adopter and traceable to moderation. Not called by `rescue_actors_sync`: a character moved out of a deleted world was not adopted from anybody. [US6] Record an adoption per copied entity in `src/server/src/collections/copy.rs`, inside the existing transaction, and update the module header — its current text says the copies carry no source id and that is about to stop being true
- [X] T047 **Done** — all three, inside each copy's transaction; `a_copy_made_through_an_item_link_is_reached` proves the item path by behaviour, through a real link and a real copy. [P] [US6] Record adoptions in the three singleton copy impls in `mutations_actor_shares.rs`, `mutations_item_shares.rs` and `mutations_ability_shares.rs`
- [X] T048 **Done** — plus `fan_out_resolve` and `tell_of_restoration_sync`; each fan-out is one transaction of its own, so a takedown never reaches half the copies. `CONTENT_DISABLED_AS_COPY` is also a GraphQL enum value, because `to_graphql_case` refuses an action type it does not know and an administrator reading a child case would otherwise get "Unknown action type". [US6] Create `src/server/src/moderation/reach.rs` with `fan_out_disable` and `fan_out_forward` per `contracts/takedown-reach.md`, and add `content_disabled_as_copy` to the action types and to `is_disabled_status` in `src/server/src/moderation/mod.rs`
- [X] T049 **Done** — and `resolveModerationCase` refuses `CONTENT_DISABLED_AS_COPY` as a resolution (a copy is disabled by its source's takedown, never by hand). `an_administrators_resolution_reaches_the_copies` covers both directions; `a_counter_notice_brings_the_copies_back_with_the_source` asserts the copy is forwarded with the source's exact date. [US6] Call `fan_out_disable` from `submit_takedown_notice_impl` and `fan_out_forward` from `submit_counter_notice_impl` in `src/server/src/graphql/mutations_moderation.rs`, and fan `resolveModerationCase` to child cases — the source coming back while its copies stay dark is the failure FR-023d exists for
- [X] T050 **Done** — one notice per adopter however many copies, naming their own copies and nothing about the notice (a test checks the claimant, source and sharer never appear). `share_taken_down` goes to the sharer only when the takedown reached copies; `strike_recorded` already tells them about the source. The words live in the new `pages/user/noticeText.ts`, moved out of `StandingPage.tsx` so they could be tested (`noticeText.test.ts`: not accused, not a strike, not deleted, plurals). The kinds were already in the `account_notices` CHECK. [US6] Write `account_notices` rows on fan-out: `adopted_copy_disabled` to each adopter, `share_taken_down` to the sharer (FR-023b, FR-024). The adopter's wording must make clear they are not accused of anything — a copy-writing requirement with a test
- [X] T051 **Done** — in the lazy restoration's own blocking task, and also when an administrator restores a copy's case directly. [US6] Write `adopted_copy_restored` when a lazy restoration materialises in `src/server/src/moderation/mod.rs`'s `effective_status` path
- [X] T052 **Done before this phase, 2026-09-07** — both call sites pass `ModerationEntityType::WorldLoreEntry.as_db_str()`. [US6] Fix the entity-type string in `src/server/src/lore_sync/plan.rs:109` and `src/server/src/lore_sync/incoming.rs:242` (`"lore_entry"` → `"world_lore_entry"`), closing T043
- [X] T053 **Already true** — the three singleton reads refuse a disabled target with `UNAVAILABLE` through `effective_status`, and the collection read withholds a disabled member through `resolve_member`. A disabled copy is an ordinary disabled entity, so both apply to it unchanged. [US6] Refuse a share link whose target is disabled, on the existing one-message rule `UNAVAILABLE` already uses, in all four `mutations_*_shares.rs` read paths (FR-022)
- [X] T054 **Done** — two people in two contexts: `user1` shares a lore entry and an ability, `user2` adopts them, renames the copy and writes a note beside it; a takedown leaves the copy a moderated placeholder, the note untouched, `user2`'s standing page saying "You are not accused of anything" with no strike, and the link withholding the entry; a counter-notice forwards source and copy with one date (the `RETURNING` count of two) and both come back, the rename intact, with "is back" on the standing page. Its first run failed on the test's own two mistakes — a rename regenerates a lore slug, and spec 015's placeholder carries the notice in the title, not the body. [US6] Add `apps/web/e2e/takedown-reach.spec.ts` covering quickstart Scenario F end to end, including the adopter's untouched neighbouring work (FR-023c) and restoration without asking (FR-023d)
