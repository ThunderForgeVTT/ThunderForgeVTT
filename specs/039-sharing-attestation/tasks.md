...
- [ ] T001 [P] Write ADR-093 (the sharing attestation record) in `docs/adrs/20260907-080-the_sharing_attestation_record.md` — terms compiled into the server with `include_str!` on the precedent at `src/server/src/admin.rs:37`, versioned by content hash, archived in `terms_versions` at startup, recorded with no foreign key to `users` on the precedent `content_moderation_actions` set
- [ ] T002 [P] Write ADR-095 (account standing and the termination window) in `docs/adrs/20260907-082-account_standing_and_the_termination_window.md` — standing derived from the existing counting rather than stored, the window as the only row, the sweep as the fifth `spawn_*_task`, and `MODERATION_TERMINATION_REQUIRES_HUMAN` defaulting to true
- [ ] T003 [P] Write ADR-096 (the operator acknowledgement) in `docs/adrs/20260907-083-the_operator_acknowledgement.md` — an instance attesting on the same record as a person, why `instance_identity` is not reused, and the boundary with spec 040
- [ ] T004 **GATE** Write ADR-094 (adoption provenance and the reach of a takedown) in `docs/adrs/20260907-081-adoption_provenance_and_the_reach_of_a_takedown.md`, carrying the guardrail determination in plan.md, and **obtain the accountable owner's acceptance as ADR-069 has**. It reverses spec 026 FR-012's "no referential link back to the source" (`src/server/src/collections/copy.rs:10`) and amends ADR-069. If declined: strike FR-022–FR-023d and SC-006/SC-008 from `specs/039-sharing-attestation/spec.md`, delete Phase 6 from this file, and record the decision in the ADR as rejected
...
- [ ] T011 Create `src/server/src/legal/mod.rs` — `include_str!("../../../../legal/sharing-terms.md")` and the operator statement, normalisation (strip the leading HTML comment, trim) matching `apps/web/src/legal/legalDocuments.ts`'s `sectionsOf`, `sha256` version identity, and section splitting; with unit tests asserting a prose edit changes the identity and a comment edit does not
- [ ] T012 Create the Diesel migration `src/server/migrations/2026-09-07-130000-0000_terms_versions_and_attestations/up.sql` and `down.sql` for `terms_versions` and `attestations` per data-model.md §§ 1–2, following the comment style of `2026-09-05-000000-0000_create_world_collections/up.sql` and stating why `attestations` has no FK to `users`
- [ ] T013 Run `make schema` to regenerate `src/server/src/schema.rs`, and add `TermsVersion`/`NewTermsVersion` and `Attestation`/`NewAttestation` to `src/server/src/models.rs`
