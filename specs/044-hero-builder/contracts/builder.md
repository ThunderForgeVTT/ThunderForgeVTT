...
what the server learns (§4). The rules a task is measured against are B1–B9 in
§5. Nothing here changes `storage/svg.rs`, the WebP path, the asset routes or
the engine.

## 1. `packages/heroes` — five additions in phase (a), one in phase (d)
...
export const HERO_LABELS: {
  fields: Record<string, string>;   // every key of HERO_PARTS, HERO_COLORS,
                                    // HERO_FLAGS, plus name, title, size
...
// the closed lists and HERO_PALETTES. A locked field is absent from the result,
// so the caller's own value survives a merge. No model, no service, no
// Math.random.
...
### The imagery panel (phase b, FR-023 to FR-025, FR-033)

`ActorImageryPanel` gains one control, `data-testid="actor-imagery-build"`,
rendered exactly when `canEdit` is true — the same condition that already
governs the file inputs (`ActorImageryPanel.tsx:175`). It opens
`HeroBuilderDialog`, which `import()`s the library at open time and nowhere else
(FR-022, SC-012).
...
The panel is the only place the control lives, so it arrives on
`NpcEditorPage` (`:224`) and on `ActorDetailPage` together, wherever the panel
is mounted.

### Quick NPC (phase b, FR-027, FR-028)

`data-testid="quick-npc"` beside the compendium's existing Add-NPC link
(`NpcCompendiumTab.tsx:261-270`), offered on the same condition. The dialog
...
listed as lacking art, with a way to add it.
...
type GraphQLWorldActor { …, artLocked: Boolean! }
```
...
| **B9** | A collection copy carries each image's spec; a personal export carries it; a shared-actor copy carries no images and so carries no specs. | `collections/copy.rs:597-616`, `users/export_content.rs:304-314` | d |
