...
- [X] T011 **Stale as of 2026-09-03; it exists.** The entry said no `declineResourceTrade` mutation existed server-side and no UI offered anything beyond accepting. Both are there: `decline_resource_trade_impl` in `packs/systems/genie/server/src/session/mutations.rs` with its own two-party test, `declineResourceTrade` in `useGenieSession`, and a **Decline** button beside Accept in the pack's own `SessionResourceTrade.tsx`. Rejecting a proposal outright — as distinct from declining it — remains out of scope, which is what the original entry conflated
- [ ] T012 Session Resource trading across two **simultaneously connected** clients, each seeing the other's move live. The stated blocker — "no GraphQL subscription" — is stale: `useGenieSession` calls `subscribeToWorldEvents` and refetches trades on a trade event. What is missing is the test proving it, and that is worth writing rather than assuming, because the wiring existing and the wiring working are different claims
