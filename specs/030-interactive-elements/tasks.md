...
- [x] T080 [P] Document the feature in `docs/interactive-elements.md`, including what open, closed and locked mean, and how a subsystem contributes an effect
- [x] T081 [P] Update `./MVP.md` with what this delivers and what it deliberately does not — party tokens and multi-scene navigation remain unbuilt
- [x] T082 Measure a scene with 50 interactives against the documented baseline via `apps/web/e2e/engine-limits.spec.ts` and record the figure. The expected answer is "no measurable change"; an expected result that was never checked is an assumption (SC-007)
- [x] T083 Run `specs/030-interactive-elements/quickstart.md` end to end, including the manual walkthrough — steps 5 and 8 are the ones worth doing by eye
- [x] T084 Run `pnpm verify` (`./scripts/verify.mjs` — rustfmt, clippy, prettier, eslint) and fix what it reports **in the code this feature added**. Keep it to that: a repo-wide lint remediation folded into a feature phase buries the feature work. `pnpm verify:fix` rewrites what can be rewritten mechanically

---

## What was left, and why

**`pnpm verify`'s `web lint` step still fails**, on 19 warnings in files this
feature did not touch: the two other capacity specs, four torture specs, four
`ui/` components, two context providers, `useCanvasEngine.ts` and
`WorldPage.tsx`. `git log` confirms none of them changed in this spec's
commits. Everything spec 030 added passes lint, format, clippy, the SDK
generation check and the new seam check.

That failure is the repo-wide lint pass already deferred as its own piece of
work. T084 says to keep this phase to the code the feature added, and the
reason holds: a repo-wide remediation folded into a feature phase buries the
feature work in an unreviewable diff.

**T082 measured a sibling spec rather than a level inside `engine-limits`.**
`apps/web/e2e/engine-interaction-limits.spec.ts`, following the shape
`engine-status-limits.spec.ts` established — it sweeps one axis under two
conditions in one session, so the answer is a cost rather than a frame time
next to a baseline recorded on a different build. Result: ratio **0.994** with
200 moving tokens and 50 region interactives, both sides on the vsync floor.

**Two contributors moved out of `interaction.rs`.** T031 and T067 named that
file for `lore.open` and `nav.request_scene`; they are in `lore_link.rs` and
`navigation.rs` instead. The tasks' intent was "the first contributor" and "the
contributor whose effect is honest about being incomplete", both preserved —
but declaring them in the interaction core would have made that core name two
effects, which is the one thing FR-039 forbids and `check-interaction-seam.mjs`
now checks.

**T083 found two wrong commands in its own quickstart.** Step 2's filter
(`registry_`) matched two removability tests and _not_ the collision test the
step exists for, and step 7 pointed at the token-count sweep rather than the
interactives measurement. Both fixed. This is the same trap spec 029 hit, where
a filter matching zero tests reported a green run for a check that never
happened — the lesson recorded there is to read the count, and it earned its
place again.
