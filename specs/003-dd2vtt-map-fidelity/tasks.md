...
- [X] T003 [P] [US1] Playwright e2e: a wall's "Blocks movement" toggle reaches an already-connected second session with no reload (FR-005, SC-002). **Green as of 2026-09-03.** It stood as `test.fail` because nothing in `apps/web` opened a live subscription — `startWallEventSync`/`applyWallWorldEvent` existed with no transport feeding them. `WorldPage.tsx` now opens `worldEventsCreated` over `graphql-ws` and drives every canvas primitive from it (walls are event code 10), so the annotation did exactly what its closing line asked: a later fix flipped it green rather than the gap going unnoticed. Passing in the sweep as `map-editor-tooling.spec.ts:381`
