...
A GM starts a scene with no imported map at all — just a background, or nothing yet — and, using the same canvas the players see (not a separate offline editor), places walls, toggles doors, marks specific wall segments passable (e.g. a window, a broken section of fence, a low wall — visually blocking sight but not movement, or vice versa), and places torches (light sources) directly, entirely by hand. This isn't a one-time setup step: a GM mid-session, realizing a corridor is missing a door or a room needs a torch for atmosphere, makes the tweak on the spot without pausing the game, restarting the server, or leaving the scene. Only the GM (the scene's owner) can do this; players never see these controls, only their effects (a wall blocks their vision, a lit torch illuminates a room, a passable segment lets a token walk through it).

**Why this priority**: This is the actual headline capability this spec exists to deliver — full from-scratch map construction, not just import-then-verify. Most of the underlying pieces already exist (spec 001/002 delivered hand-drawn wall/door/shape authoring and light-source placement, all GM-gated and live-synced), so this user story's job is to confirm the full "walls + doors + passability tweaks + torches, all mid-session" experience actually holds together end-to-end, and to close the one clearly-missing piece: a GM cannot currently set a wall segment's passability directly — only the door open/closed cycle exists as a UI affordance, even though the underlying data already supports movement- and vision-blocking as independent properties.

**Independent Test**: On an empty scene (no import), a GM places a wall, toggles part of it into a door, right-clicks a different wall segment to mark it passable, and places a torch (light source) — each individually, using only the live scene canvas — and confirms each appears immediately with correct behavior (wall blocks vision/movement; door is passable only when open; the passable segment lets tokens through regardless of door state; torch lights the area) and is visible with correct behavior to a second, concurrently-connected player session, without any server restart or page reload — independently verifiable without the round-trip/field-disclosure work in User Stories 2-3.

**Acceptance Scenarios**:

1. **Given** a GM on an empty scene, **When** they draw a wall and toggle part of it into a door, **Then** both appear immediately and behave distinctly: the wall blocks vision and movement, the door blocks only when closed.
2. **Given** a GM has an existing wall segment (with or without door state), **When** they right-click it and toggle its passability, **Then** it immediately becomes (or stops being) traversable by tokens, independent of whether it's a door and independent of whether it still blocks vision.
3. **Given** a GM on any scene, **When** they place a torch (light source) at a point on the canvas, **Then** it immediately illuminates the surrounding area for every connected session, GM and players alike.
4. **Given** an active session with a player connected and viewing a scene, **When** the GM adds a wall, toggles a door, marks a segment passable, or places a torch mid-session, **Then** the change appears for the player within a few seconds, with no reload, no disconnect, and no interruption to anything else happening in the session.
5. **Given** a player (non-GM) viewing a scene that has authoring controls available to its GM, **When** the player looks at the same scene, **Then** they see none of the wall/door/passability/torch authoring controls — only the rendered results (walls blocking vision, lit areas, closed doors as obstacles, passable segments as crossable).
6. **Given** a GM wants to start a brand-new scene with no imported map, **When** they create it, **Then** they can immediately begin placing walls/doors/torches and toggling passability on it without first needing to import a `.dd2vtt` file — an empty or background-only scene is a fully valid starting point, not a degraded one.

---

### User Story 2 - A GM trusts that an imported or hand-built map is exactly what they see after any reload (Priority: P1)

A GM imports a `.dd2vtt` map into a scene, or builds one from scratch by hand (User Story 1): walls, doors, passability tweaks, light sources, and the background image all appear correctly. Days later, the GM (or a player) reloads the page, the server restarts, or the GM switches away from the scene and back. Today, nothing actually verifies that the scene the GM sees after any of these events is identical to what was there before — only that parsing a fixture file "succeeds" and produces the expected *counts* of walls/lights. A GM needs confidence that their table's map is durable: what's on it is what stays there, indefinitely, whether it arrived via import or by hand.
...
3. **Given** an imported map's walls and shapes are then edited by a GM through hand-drawn authoring (spec 002/User Story 1) — a wall added, a segment's passability toggled, a shape deleted — **When** the scene is reloaded afterward, **Then** the reloaded state reflects the edits exactly (the edited/hand-built state is itself durable, not just the originally-imported state).
...
- What happens if a non-GM (player) attempts to invoke wall/door/passability/torch authoring directly (e.g. a crafted request, not through the UI)? Must be rejected server-side, not just hidden client-side — consistent with the existing GM-only enforcement already in place for walls/shapes (spec 001/002) and light sources.
...
- What happens to an in-progress wall chain if the GM's browser tab loses focus or the connection drops mid-draw? Same as existing wall-authoring behavior (spec 002): nothing partial is persisted.
- What happens when a GM toggles passability on a wall segment that also has a door state set (open/closed)? The two controls act on the same underlying segment — planning must define the precedence (e.g. does toggling "passable" on a closed door make it act open regardless of its visual door state, or is the passability toggle only meant for non-door segments?) rather than leaving it ambiguous; not resolved at the spec level since it's a UI/interaction-design decision, not a scope decision.
...
- **FR-001**: A GM MUST be able to right-click (or equivalent direct interaction) an existing wall segment and toggle whether it currently blocks movement ("passable" vs. "not passable"), independent of the segment's door state, using the same direct-on-canvas interaction already available for drawing walls and toggling doors (spec 002).
- **FR-002**: The passability toggle in FR-001 MUST be a direct, immediate GM action — no automated skill-check, roll, or player-facing prompt (e.g. an Acrobatics/DEX check to squeeze through) is part of this feature; such mechanics are explicitly deferred, see Assumptions.
...
- **FR-004**: All wall/door/passability/torch authoring MUST be usable on a scene with no imported map at all — an empty or background-only scene MUST be a fully valid starting point for hand-built map construction, not a degraded one requiring an import first.
- **FR-005**: Every wall/door/passability/torch change a GM makes MUST propagate to every other connected session (GM or player) viewing the same scene within a few seconds, without requiring a page reload — consistent with the existing real-time sync already used for walls/shapes/lights.
- **FR-006**: Only the scene's GM (owner, or a co-owner/invited GM per spec 002's `world_members`) MUST be able to author walls/doors/passability/torches; this MUST be enforced server-side, not only hidden in the client UI, consistent with existing GM-only enforcement for walls/shapes/lights.
- **FR-007**: A GM MUST be able to perform all of the above (add/edit/remove walls, doors, passability toggles, torches) while a session is actively in progress (players connected, viewing the scene), without needing to pause, restart, or otherwise interrupt the session.

**Round-trip durability (User Story 2)**

- **FR-008**: The system MUST provide an automated, repeatable verification that every wall (including door state and passability), light source, shape, and token belonging to a scene survives being persisted and reloaded with all field values identical to what was written — not merely that the right *number* of each entity exists.
...
- **FR-011**: The round-trip verification in FR-008 MUST also cover a scene whose data has since been modified through hand-drawn authoring — walls, doors, passability toggles, torches, shapes (User Story 1, spec 002) — confirming edited/hand-built state, not just originally-imported state, survives a reload.
...
- **Wall segment**: Already-persisted entity (spec 001/002). Its movement-blocking and vision-blocking properties are already independent columns (`blocks_movement`, `blocks_vision`), and door state is a separate field layered on top — the underlying data model already supports "blocks movement but not vision" (or vice versa) with **no schema change needed**. This feature's only net-new work here is a GM-facing control (right-click → toggle passability) that exposes `blocks_movement` directly, rather than only through the door open/closed cycle.
...
- **SC-001**: A GM can go from an empty scene to a room with at least one wall, one door, one passability-toggled segment, and one lit torch, entirely by hand, in under a few minutes, with zero import step required.
- **SC-002**: A change a GM makes to walls/doors/passability/torches mid-session is visible to a connected player within the same few-seconds window already achieved for existing wall/shape/light sync (spec 001/002) — no perceptible new lag introduced by adding the passability toggle/torches to the authoring surface.
- **SC-003**: 100% of a `.dd2vtt` fixture's walls (with door state and passability), lights, and background image reference match exactly after a persist-and-reload cycle, verified by an automated check that runs on every relevant code change, not just manually.
...
- **The "edit mode" the user description asked for already exists implicitly, not as a separate mode to build.** GM-only authoring tools (walls/shapes/lights) are already always available to a scene's owner whenever they're viewing it (spec 001/002) — there is no separate "play mode" a GM has to leave to start editing, and this spec does not introduce one. User Story 1 is about closing the passability/torch-as-scratch-authoring gaps within that existing always-available-to-GM model, not about building a new mode-switching UI. If planning finds a real product reason to add an explicit mode toggle (e.g. to visually distinguish "I am about to edit the map" from normal play), that's a scope decision for `/speckit-plan`, not assumed here.
- **"Windows" are not a distinct wall type or entity.** An earlier draft of this spec proposed a separate "window" wall-state; the actual desired capability (confirmed directly) is simpler: a GM can right-click any wall segment and toggle its passability (movement-blocking) independent of its vision-blocking and door state, which the existing `walls` table already supports via independent `blocks_movement`/`blocks_vision` columns — no new entity, enum variant, or migration is implied. "Window," "broken fence," "low wall," etc. are just narrative descriptions of what a passable-but-sight-blocking (or vision-passable-but-movement-blocking) segment represents to a GM, not a system concept.
- **Skill-check-gated passability (e.g. an Acrobatics/DEX check to squeeze through a gap) is explicitly future work, not in this spec.** The passability toggle (FR-001/FR-002) is a direct, manual GM action only — no automated roll, prompt, or player-facing check mechanic is introduced here.
