...
- [X] T007 [P] [US1] Playwright e2e: a client's own change never visibly double-applies, flickers, or reverts when it round-trips back via the subscription (FR-007; SC-002; quickstart.md Scenario 1 step 5) — `apps/web/e2e/canvas-authoring.spec.ts`, "a session's own wall does not double-apply, flicker or revert when its event echoes back". The author draws a wall, selects it, and waits 5s — well past any plausible round trip through Postgres, the broadcast channel and back down its own subscription. Two independent assertions: the selection survives (an echo that re-spawned the wall would drop it), and the server reports exactly one stored wall (an echo that spawned a second would leave the selection intact but the scene holding two). The count was mutation-tested by drawing a second wall: it reads 2, so it tracks the scene rather than always agreeing.
