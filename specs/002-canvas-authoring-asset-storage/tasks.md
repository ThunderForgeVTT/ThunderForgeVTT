...
- [ ] T023 [P] [US3] Playwright e2e: paste-to-canvas happy path, image appears within 10s, survives reload, visible to a second (player) session in `apps/web/e2e/canvas-authoring.spec.ts` (quickstart.md Scenario 3; SC-004; US3 Acceptance Scenarios 1, 2, 4) — **not started**
- [ ] T024 [P] [US3] Playwright e2e: pasting a non-image clipboard item is silently ignored, no upload attempted (Edge Cases; US3 has no numbered scenario for this but spec.md Edge Cases calls it out) — **not started**
...
- [ ] T030 [US3] Add `AssetPasteTool/` to `apps/web/src/components/canvas-tools/` (matching `WallTool/`/`ShapeTool/` directory convention): clipboard-paste listener scoped to focused canvas, calls `uploadCanvasImage`, shows upload/error state (FR-011, FR-013's "clear error") — **written and compiles (`tsc`/lint clean), not runtime-verified** — no live paste-to-canvas test was run against the actual UI
- [ ] T031 [US3] Spawn a placed-image entity in the engine for each `CanvasImageAsset` on the active scene, following the existing `BackgroundPlugin`/`sync_scene_background` pattern (`src/engine/src/systems/background.rs`) generalized to non-background pasted images — new resource/system pair, not a new plugin (Constitution Principle I/II) — **written and compiles (`cargo check --target wasm32-unknown-unknown` clean), not runtime-verified**
- [ ] T032 [US3] Frontend loading/error state for asset fetch failures (RustFS temporarily unavailable) per Edge Cases: previously-cached images continue to display; new fetches show a clear loading/error state — in the engine's asset-loading path and/or `AssetPasteTool/` — **partial**: `AssetPasteTool` has an error/status indicator, but the "previously-cached images continue to display on RustFS outage" behavior was not runtime-verified
...
- [X] T039 [P] Run `cargo check --target wasm32-unknown-unknown` on `src/engine` and native `cargo check`/`cargo test` on `src/server`, resolving any new warnings (Constitution Principle V) — both clean, no new warnings vs. baseline (7 engine, 29 server); server: 60/60 tests passing (56 spec-002 + 4 new IDOR-fix regression tests)
- [ ] T040 [P] Run `tsc`/build on `apps/web` and execute the full `apps/web/e2e/canvas-authoring.spec.ts` suite (Constitution Principle V) — **partial**: `eslint` clean on all changed frontend/e2e files; full-project `tsc --noEmit` has ~40 pre-existing unrelated errors (RxDB/vitest module resolution) confirmed not caused by this feature. Live suite result: 5/7 passing (map-import baseline + all 3 wall tests + cross-session sync); the 2 shape tests (T016/T017) fail — see those tasks for detail. Not markable done until the shape tests pass in a clean environment.
- [ ] T041 Execute quickstart.md Scenarios 1-5 end-to-end against a freshly provisioned local stack (`docker compose up` only), confirming SC-008 — **not run as a single formal pass**; Scenarios 1 (walls) and 4/5 (RBAC isolation, dev provisioning) are covered piecemeal by the passing automated tests above, but no one did a clean `docker compose up` from a fresh checkout and walked quickstart.md step-by-step end-to-end in this session
