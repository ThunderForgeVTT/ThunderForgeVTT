...
- [X] T016 [P] [US2] Playwright e2e: freehand, rectangle, ellipse, line/arrow and text shape creation + persistence in `apps/web/e2e/canvas-authoring.spec.ts`. **Verified green 2026-09-03** (`canvas-authoring.spec.ts:695`). The blocker was environmental and correctly diagnosed — GPU contention on the authoring machine producing `boundingBox()` timeouts and real "GPU stall due to ReadPixels" warnings, not an app bug. It passes on a quiet machine, in the full sharded sweep, alongside every other canvas spec
- [X] T017 [P] [US2] Playwright e2e: scene-switch isolation and GM/player delete visibility (SC-003). **Verified green 2026-09-03** (`canvas-authoring.spec.ts:758`). The `WorldPage.tsx` fix recorded here — dispatching `remove_wall`/`remove_shape` with `source: "sync"` for the outgoing scene before loading the next, so a prior scene's entities stop lingering spawned and hit-testable — is confirmed live rather than only code-reviewed
