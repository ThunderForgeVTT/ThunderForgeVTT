...
- [~] T056 [P] [US3] E2E: corrupted blob and orphaned index entry both self-repair silently (SC-005). **Two of three legs green; the third is blocked on a finding, not on the test.** Green: a blob no row refers to is reclaimed while an unfinished one is left alone (it may be another tab's write), and a row naming a blob that is gone is dropped and the asset comes back. That second one records the convergence honestly — recovery takes **two opens**, because `run_sync` builds its manifest *before* calling `repair_world`, so the open that finds the lie has already told the server it holds the item. Both also assert `pageerror` silence, which is the half of SC-005 that was previously assumed rather than checked. Blocked (`test.fixme`, with the measurement in the test): a *corrupt* blob is never read, because `try_cached` declines when the cache is not ready or no fingerprint is published, and on a fresh page load the scene background is both — the sprite load is dispatched when scene state arrives, well before `sync_world_cache` resolves. Console ordering shows the background `GET 200` landing before `canvas asset cache ready`, and the forged file surviving byte for byte, which a read would have discarded. **So the scene background is stored by the prefetch and never read back** — the largest asset in a world, currently costing a write and saving nothing. Finishing the leg means loading an asset *after* the first sync (a mid-session scene switch), where `is_ready()` and `fingerprints` both hold; whether the background itself should be cache-served is a design question that trades SC-002 against SC-001, in `apps/web/e2e/world-cache-repair.spec.ts`
