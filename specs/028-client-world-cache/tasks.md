...
- [X] T099 [US7] Implement discrepancy detection: compare client-reported outcomes the server can independently determine (dice first, per ADR-044) against the server's own result, in `src/server/src/graphql/mutations_reconcile.rs`. The roll record is read filtered by **both** id and `world_id`, so a record from another world reads as "no such row" rather than as a value to compare against. Values within `1e-6` are equal — a JSON round trip is not a discrepancy, and dice totals are integers so nothing genuinely different lands inside that window
...
- [X] T102 [US7] Accept an attributed submission only when the server confirms the submitter holds the GM role, using the existing role check; reject attributed submissions from non-GMs (FR-061, FR-061a), in `src/server/src/graphql/mutations_reconcile.rs`. The whole check is `subject_user = attributed_to.unwrap_or(submitter)` plus a role test on the already-resolved membership — no keypairs, no signature format, no new trust root, exactly as ADR-052 argues. The negative test submits the identical change on a token the submitter **owns**, so attribution is the only thing that can be refusing it. The emitted event carries `submitted_by` only when a change was relayed, so the ordinary case is byte-identical to before and the field says who relayed it, never that anything was wrong (FR-061b)
- [X] T102a [US7] Render a discrepant result distinctly in the GM's view only, inspectable to show claimed vs determined values — display treatment, **no resolution workflow, no escalation, no dispute process** (FR-065, FR-065a, FR-066, FR-067), in `apps/web/src/components/world/RollResult.tsx`. **The register is the work.** A red badge would satisfy "rendered distinctly" and be the wrong answer: it makes the software the accuser in a situation where it cannot know, and puts an innocent player under suspicion in front of the one person who can act on it. So it is a *difference, noted* — ordinary weight, ordinary colour, a dotted underline and one muted word, opening to two numbers and a sentence saying nothing was changed. `isGameMaster` defaults to **false**, so a caller that has not thought about roles shows an ordinary result rather than leaking a mark to the table. `discrepancyToShow` refuses again at the display: not the GM, no record, either value missing or non-finite, or equal — all silent. The `Number.isFinite` guard is load-bearing, since `NaN !== NaN` would otherwise make every unparsed value a discrepancy, the loudest false positive available
- [X] T102c [US7] Report a discrepancy only on a genuine determined-value mismatch — never on timeout, parse failure, version mismatch, or any ambiguity; when in doubt report nothing (FR-067a), in `src/server/src/graphql/mutations_reconcile.rs`. **Enforced by a type, not a branch.** `GraphQLDiscrepancy` is constructible only inside `discrepancy_between`, whose first line is `let DeterminedValue(determined) = determined?;`, and the private `DeterminedValue` is minted at exactly one place: `Ok(Some(finite))` from the roll lookup. So a timeout, an absent row, a cross-world row, a parse failure and a version mismatch all leave by the *same* `None` as "determined and equal" — neither can be made loud without making the other loud too. A false discrepancy puts an innocent player under suspicion in front of the only person who can act on it; a missed one costs nothing, which is why the asymmetry is built into the shape rather than trusted to a reviewer
- [X] T102b [US7] Report no discrepancy where the server has no independent basis for the outcome, e.g. ordinary token movement (FR-068), in `src/server/src/graphql/mutations_reconcile.rs`. Falls out of the same shape: with no reported outcome at all the leading `reported?` returns before any lookup runs. Absence of evidence is not a flag
- [X] T103 [US7] Submit peer-adjudicated changes for confirmation and re-authorization on reconnection, reverting locally and informing the originator on rejection (FR-062), in `apps/web/src/engine/world/sync/index.ts`. **Extends the Phase 9 outbox rather than adding a second queue** — adjudication is provisional and re-authorized with the server's decision final, which is the sentence FR-042 already writes about an offline edit. A parallel queue would need its own answer to interruption, its own drain and its own conflict order, and the two would drift apart in exactly the situation neither is tested in. What adjudication adds is that the submitter may not be the author, so the originating user is carried *inside the stored command* — the outbox is the only thing in the path that survives a reload. Reading it back is strict: anything malformed is `null`, because naming the wrong person is worse than naming nobody. The revert is a **re-read of the server's state, not an undo**: the client cannot reconstruct the prior value, since offline play may have moved the token repeatedly and the change may have originated on another machine
- [X] T104 [US7] Handle the server returning mid-adjudication without double-applying or dropping a change, in `apps/web/src/engine/world/sync/index.ts`. **Found a live double-submit.** `WorldPage` reconciles from both the heartbeat recovering and the socket going live, and an ordinary reconnect fires both — each read the same queue and submitted it. A per-world in-flight map makes the second caller a reader of the first's answer, which is what it wanted anyway. The drop half is subtler: an edit made *between* the queue being read and the outbox being drained decided to queue while `shouldQueue` was still true, and lands after the drain has gone past it — submitted once, it would sit there until the next disconnection, which on a stable connection is never. The drain re-reads until a pass produces nothing new, bounded at three. Both Phase 9 rules survive verbatim: a change leaves the outbox only once the server has spoken about it, and an unanswered change stays queued rather than being reported as handled
...
- [X] T116 [P] [US8] Implement a low-priority prefetch queue drawing only on the caller's own `SyncPlan`, confined to the open world (FR-069, FR-072, FR-073) in `crates/thunderforge-cache-browser/src/lib.rs`
- [X] T117 [US8] Yield prefetching to the active scene, user-initiated fetches, and live updates (FR-070) in `crates/thunderforge-cache-browser/src/lib.rs`
- [X] T118 [US8] Stop prefetching rather than evicting — speculative content must never displace content the user actually has (FR-071) in `crates/thunderforge-cache-core/src/budget.rs`
...
- [ ] T125 [P] Backfill `content_hash` for existing assets via a background job in `src/server/src/storage/`, relying on NULL-means-fetch so it need not complete before release. **Sized against the dev database 2026-08-28:** 542 of 1601 rows lack a hash, but they are two unrelated populations and only one is real. 413 are `Background` (1432 MB, average 3.5 MB, largest 13 MB, spread over 412 worlds) — the genuine population left by the map-import bug fixed in `4ea412e`; the newest is dated 2026-08-27 22:20, i.e. none have accrued since the fix, which is the evidence that the leak is closed rather than merely patched. The other 129 are `Pasted` and are **test noise**: `world_sync_plan.rs`'s `insert_asset` helper takes `content_hash: Option<&str>` and deliberately writes NULL rows to exercise the NULL-means-fetch path, and the e2e suite runs against this same database. The real upload path (`mutations_assets.rs`) has always set the hash. So the job's actual work is re-reading ~1.4 GB of background art from object storage to hash it — which is exactly why it is a background job that need not finish before release, and why it should be paced rather than run as one pass
