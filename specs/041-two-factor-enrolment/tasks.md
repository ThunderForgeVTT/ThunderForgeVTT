...
- [X] T007 **Closed 2026-09-10 as superseded — see the note at the end.** Create the Diesel migration `src/server/migrations/2026-09-07-100000-0000_two_factor_enrolment/up.sql` and `down.sql` creating `two_factor_enrolments` (with `UNIQUE (user_id)`) and adding `users.two_factor_last_used_step` and `users.two_factor_required_by` per data-model.md §§ 1–2. The `100000` timestamp is deliberate: spec 036's work already holds `2026-09-07-000000-0000_session_description`, and Diesel keys a migration on that leading version, so two directories sharing it collide — **superseded.** The pending enrolment landed as columns on `users` (`two_factor_pending_secret_encrypted`, `two_factor_pending_started_at`, migration `2026-09-07-090000`) rather than a `two_factor_enrolments` table. One pending enrolment per account is exactly what a `UNIQUE (user_id)` table would have enforced, and the columns cannot drift from the row they belong to
...
- [X] T009 Create the Diesel migration (landed as `2026-09-09-110000-0000_two_factor_events`) `up.sql` and `down.sql` creating `two_factor_events` with its `event_type` CHECK and `actor_user_id … ON DELETE SET NULL`, and adding `purpose` and `failed_attempts` to `login_two_factor_challenges`, per data-model.md §§ 4, 6. Carry spec 035's comment forward: no identifier column, and none may be added
- [X] T010 **Closed 2026-09-10: everything that exists is done; the one missing model belongs to a table T007 decided not to create.** Regenerate `src/server/src/schema.rs` and add the `TwoFactorEnrolment`, `UserRecoveryCode` and `TwoFactorEvent` `Queryable`/`Insertable` pairs to `src/server/src/models.rs` — **partly superseded.** `schema.rs` is regenerated and `UserRecoveryCode` and the `two_factor_events` pair exist; `TwoFactorEnrolment` does not, because T007's table does not (see above)
...
- [X] T012 [P] Create `crates/thunderforge-axum-auth-core/src/recovery_codes.rs` — generation over `random_setup_code()`'s alphabet, normalisation (case-fold, strip hyphens and whitespace), and the constant-work match rule — with a proptest that the number of hash comparisons does not depend on whether or where a code matched — **landed as `src/server/src/auth/two_factor/recovery.rs`**, not a crate module. Generation, normalisation and the constant-time compare are all there and tested; the address differs from the plan because recovery needs the pool and the crate admits nothing that does
...
- [X] T021 **Closed 2026-09-10 as a decided deviation — not done, on purpose, for the reason below.** [US1] Remove `TwoFactorSetupStartRequest`/`TwoFactorSetupConfirmRequest`'s `username` and `password` fields in `src/server/src/auth/types.rs` and add the ticket and QR-matrix response shapes — **not doing, deliberately.** `TwoFactorSetupStartRequest` keeps `username`/`password` because FR-001a's three entrances do not all have the same thing to prove with: account settings and first-run setup send credentials, a sign-in caught by a policy sends the challenge it was just handed. `authorise_enrolment` accepts exactly one of the two and refuses both or neither. The plan assumed one authorisation; the requirement asks for three
- [X] T022 [P] [US1] Create `apps/web/src/components/two-factor/QrMatrix.tsx` — inline SVG rectangles from the matrix, with a vitest case in `apps/web/src/components/two-factor/__tests__/QrMatrix.test.tsx`. No `dangerouslySetInnerHTML`, no dependency — **landed as `apps/web/src/components/security/TwoFactorQrCode.tsx`** — inline SVG rects from the matrix, with the server half added 2026-09-09 (`src/server/src/qr.rs`)
- [X] T023 [US1] Create `apps/web/src/components/two-factor/EnrolmentFlow.tsx` — the one flow: QR beside the grouped copyable secret, the confirming code, retry without re-scan, then the codes. It takes where-to-go-afterwards as a prop and nothing else differs between entrances (FR-001a) — **landed as `apps/web/src/components/security/TwoFactorEnrolmentSteps.tsx`**, rendered by `TwoFactorEnrolmentPanel` from all three entrances (FR-001a)
...
- [X] T031 [US2] Add `POST /authentication/2fa/recovery-codes` (session + possession) in `src/server/src/auth/two_factor/recovery.rs` and wire it in `src/server/src/auth/two_factor/mod.rs` — route exists at `/authentication/2fa/recovery-codes`, session + possession, in `two_factor/recovery.rs`
- [X] T032 [US2] Report `recovery_codes_remaining` and `recovery_codes_low` on the session response in `src/server/src/auth/sessions.rs`, so a person is told at sign-in and not only if they visit a settings page (FR-011) — reported from `build_session_response`, the one place every successful session response is built, so no sign-in path can be the one that forgets. An account with **no** second factor reports `None` rather than `0`: those are different facts, and "0 codes left" shown to somebody who never enrolled would be alarming and wrong
- [X] T033 [P] [US2] Create `apps/web/src/components/two-factor/RecoveryCodeSheet.tsx` — shown once, copyable, downloadable, with an explicit acknowledgement before it can be dismissed — the sheet is part of `TwoFactorEnrolmentSteps` rather than a separate component — shown once, and `two-factor-recovery-code` is asserted ten-to-a-set by `two-factor-enrolment.spec.ts`
...
- [X] T046 [P] [US3] Add server tests asserting `setup_status` reads incomplete while an administrator exists without a confirmed factor (FR-028) — **landed as `src/server/src/auth/setup_second_factor_gate_tests.rs`**. Only one of the two halves moved, on purpose: `setup_completed_at` is now written solely by `/setup/complete`, which refuses `409 second_factor_required`; the `admin_exists && the code was consumed` arm **stays**, because tightening it would drag every instance that completed setup before this feature back into the first-run wizard, holding a bootstrap code it cannot present again. A fresh instance is gated; an upgraded one meets the rule at its administrators' next sign-in (FR-031)
...
- [X] T048 [US3] Stop `admin_setup_basic` marking setup complete — **done by spec 040's ADR-093**; the account step is the middle of the wizard now. It still issues a session rather than a `setup` ticket, and should: the administrator has just chosen a username and password, so enrolment uses the username+password entrance ADR-082 already defines, and a fourth kind of ticket would be a credential to keep safe for no gain
- [X] T049 [US3] Defer completion until the factor is confirmed (FR-029) — **done by spec 040 in a different shape**: completion moved to `/authentication/setup/complete`, gated on `first_administrator_second_factor_confirmed`, rather than into the enrolment transaction. The recovery codes are in the confirm response either way, and the wizard has a review step after the factor that this shape keeps meaningful
- [X] T050 [US3] `setup_status` no longer equates "an administrator exists" with "setup is finished" (ADR-093), and a process death mid-wizard leaves it resumable because nothing is accumulated across steps. See T046 for why the back-compatibility arm is deliberately not tightened
- [X] T051 [US3] `admin_setup_oauth_callback` now redirects to `/setup` rather than to `return_to`. No step name in the URL: the wizard works out which step to show from the instance's own state, so this cannot go stale when the steps change. `return_to` is dropped rather than smuggled through three requests — the enrolment step ends inside the wizard, which then finishes setup
- [X] T052 [US3] Add the enrolment step to the setup wizard — **done by spec 040 as `apps/web/src/pages/setup/steps/SecondFactorStep.tsx`**, in the step list `SetupPage.tsx` builds
- [X] T053 [US3] Cover quickstart Scenario G against a fresh instance — **landed inside `apps/web/e2e/instance-setup.spec.ts`** rather than a file of its own, because the scenario *is* the setup walk and splitting it would mean running the wizard twice
- [X] T054 [US3] Enrolment completes with no mail configured (FR-001b, SC-010) — true by construction in `instance-setup.spec.ts`: the harness configures no mail, and the walk reaches a confirmed factor and recovery codes
- [X] T055 [US3] FR-030 and FR-032 are covered by server tests (`auth/enrolment_at_login_tests.rs`: `an_administrator_who_has_never_enrolled_is_taken_through_enrolment`, `taking_the_role_away_leaves_an_ordinary_account_behind`) rather than by an e2e. **There is no interface for granting or removing the role**, so an e2e would have to `UPDATE users SET is_admin` directly — which proves less than the server test does and would be the first step in this spec's suite to need a database connection
...
- [X] T067 [US6] Add the per-account requirement case to `apps/web/e2e/two-factor-admin.spec.ts` — including the boundary that matters: the account nobody named signs straight in
...
- [X] T072 [US7] Show the account's own second-factor events on `apps/web/src/pages/user/SecuritySettingsPage.tsx` (`TwoFactorHistoryPanel`), fed by a new `GET /authentication/2fa/history` that reads the signed-in account's own history and takes no user parameter to get wrong. A vitest asserts wording exists for every event type the server declares, so a new one cannot reach a security page as a raw identifier
- [X] T073 [US7] Name who can help on the challenge screen in `apps/web/src/pages/auth/LoginView.tsx`, from `support_email` — now published on `/authentication/setup/status`, which answers anonymously, because a locked-out person is exactly an anonymous caller. Where the instance has published no address, the screen says so plainly (FR-026)
- [X] T074 [US7] Add the reset control and create `apps/web/e2e/two-factor-admin.spec.ts` covering Scenario I end to end. No step reaches for a database connection, which is FR-024's own test. It also asserts what an operator must *not* get: the page after a reset carries no recovery code and no `otpauth://` secret, and the subject is not signed in
...
- [X] T079 `cargo test --workspace`: **2519 passed, exit 0**. `pnpm --filter @thunderforge/web test`: **563 passed**. Lint via `pnpm verify`, which runs `rust lint`, `engine lint (wasm)` and `file length` among its eleven checks — **all 11 passed**
- [X] T080 Full suite, `--shards=2`, rate-limit bypass on: **339 passed, 0 failed, 42.9 minutes, exit 0.** Shard 1 185/185; shard 0 153 with one failure that retried clean (33/33); the first-run lane 1/1. The one failure was real and mine — spec 039's T006 reworded a heading from "responsible for what you share" to "…publish" and `content-collections.spec.ts` asserts the words. Assertion updated to the new wording, deliberately still on the words rather than on the element: an empty box above the share button is what a broken glob produces, and that coupling is what notices
- [X] T081 `pnpm verify`: **all 11 checks passed**, nothing to fix
