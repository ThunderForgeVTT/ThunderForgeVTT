...
2. **Given** the existing `dnd5e` and `basic-game-system` packs, **When** this work is complete, **Then** `dnd5e`'s manifest has been updated to include a compliant `legal` object (derived from `research/system_dnd5e.json`), and `basic-game-system`'s manifest is populated with a minimal valid manifest (including a `legal` object declaring `"licenseName": "ThunderForgeVTT Original Content"`, appropriate for original, non-SRD-derived content) — resolved; see Edge Cases below.
...
- ~~`packs/systems/basic-game-system/system.json` is currently an empty file (a pre-existing, unrelated gap found during the compliance review); this spec treats making it schema-valid (including a `legal` object once it has content) as in scope only if/when it's populated, and does not require populating it now.~~ **Resolved**: `basic-game-system/system.json` has been populated with a minimal, generic starter-template manifest, including a compliant `legal` object (`licenseName: "ThunderForgeVTT Original Content"`, empty `trademarkRestrictions`) — see `manifest_legal_enforcement_tests::basic_game_system_json_has_a_compliant_legal_object` in `src/server/src/systems.rs`.
