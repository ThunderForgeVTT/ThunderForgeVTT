...
- [X] T024 [US1] Implement one synchronisation pass in `src/server/src/lore_sync/run.rs` — fetch, diff the world against the clone, write files, commit, push, verify. Read and write on the same pass (FR-034b).
- [X] T025 [US1] Implement commit identity in `run.rs`: **committer is `ThunderForge VTT <noreply@<instance domain>>`, author is the world member who wrote the revision** under a generated no-reply address, never a personal one (FR-017, `research.md` R6). One commit per revision in order (FR-016), with FR-020's bounded batching window for rapid successive edits.
- [X] T026 [US1] Implement rename and move as a file move preserving history in `src/server/src/lore_sync/run.rs` (FR-010), using the `lore_exported_entries` row to know which file to move rather than inferring it.
...
- [X] T038 [US1] Verify SC-011 in `apps/web/e2e/lore-repository-sync.spec.ts`: a clone renders every entry, its images and its inter-entry links **with no network access to the platform**. This is what catches absolute URLs pointing back at the app, the likeliest way FR-012 and FR-014 get half-implemented.
