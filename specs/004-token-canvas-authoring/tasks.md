...
- [X] T011 [US1] Grow `src/engine/src/plugins/token.rs` from its current 19-line placeholder into a real `TokenPlugin` chaining the (relocated, if needed) drag-input system and a visual-sync system, per Constitution Principle II and research.md §5 — **closed by spec 006** (`specs/006-token-authoring-polish`, T004-T008): `token.rs` now a real `TokenPlugin` mirroring `WallPlugin`, with drag/resize/rotate/visual-sync systems relocated into a new `src/engine/src/systems/token.rs` and chained in `Update`. Live-verified, `cargo check --target wasm32-unknown-unknown` clean.
...
- [X] T015 [P] [US2] Playwright e2e: as a connected player (non-GM), confirm no resize/rotate handles render on any token, including their own (FR-010; quickstart.md Scenario 2 step 4) — unblocked by spec 005 US4's invite/membership fix (a genuine second, distinct account can now join a world). Written and passes live (1.7m, real dev stack): a real non-owner player sees no `TokenTool` controls on any token, including one they control. **Superseded/extended by spec 006** (T003) once the keyboard-shortcut mechanism this test covered was replaced by real canvas resize/rotate handles — spec 006's equivalent regression test (`"Non-GM sees no resize/rotate controls"` describe block) confirms the same non-GM guarantee against the new handle-drag mechanism, live-passing.
...
**Checkpoint**: User Story 2 fully functional and live-verified — T013/T014/T015/T020 all pass live.
...
- [X] T021 [P] [US3] Playwright e2e: a player drags their primary token — it moves, persists, and syncs to the GM/other players; the same player attempts to drag a token not assigned to them — no effect (FR-009; SC-003; quickstart.md Scenario 3 steps 1-3) — **closed by spec 006** (T009-T011): root cause of the popover hang found and fixed, test un-skipped and confirmed passing 3/3 consecutive live runs.
- [X] T022 [P] [US3] Playwright e2e: GM grants a player control of an additional token (e.g. a summoned creature); confirm the player can now drag that token too, identically to their primary (quickstart.md Scenario 3 step 4) — **closed by spec 006**, same un-skipped test as T021.
- [X] T023 [P] [US3] Playwright e2e: a player changes their primary token's photo — visible to the GM and other players; the same player has no "create token" control anywhere in the UI (FR-009a, FR-009b; quickstart.md Scenario 3 step 5) — **closed by spec 006**, same un-skipped test as T021.

**T021-T023's combined test is now un-skipped and passing (closed by spec 006).** Two real bugs were found and fixed while writing it in this session (both kept, independent of the test's outcome at the time): (1) `TokenPanel.tsx`'s primary-token checkbox had no optimistic local state — purely server-round-trip-controlled, so a real user's click would visibly revert before snapping to the new value; fixed with an optimistic update + rollback-on-failure. (2) The test's `assignTokenOwnership` helper called `.blur()` on the owner-input field, which moves focus to `document.body` — Radix `Popover.Content`'s default outside-focus dismissal treats that as "focus left the popover" and closes it before the primary checkbox can be reached; fixed by using `.press("Tab")` instead. After both fixes, the test still hung the full 300s timeout on `primaryCheckbox.check()` — `specs/006-token-authoring-polish` (T009-T010) found the actual remaining cause via live instrumentation: the checkbox's `disabled={!token.ownerUserId}` only cleared after `handleSetOwnership`'s network round trip resolved, but Tab-driven focus traversal happens synchronously and skips a still-disabled element — so focus left `Popover.Content` before the checkbox was ever reached, and Radix's outside-focus dismissal closed the popover. Fixed with a local `ownerDrafts` map so the checkbox enables the instant a non-empty value is typed, decoupled from the mutation/refetch cycle. Test un-skipped and confirmed passing 3/3 consecutive live runs.
