...
- [~] T021 [P] [US3] Playwright e2e: a player drags their primary token — it moves, persists, and syncs to the GM/other players; the same player attempts to drag a token not assigned to them — no effect (FR-009; SC-003; quickstart.md Scenario 3 steps 1-3) — written (unblocked by spec 005 US4's invite fix), combined with T022/T023 into one `test.skip`-ed test (see note below) — not live-verified. T024/T026's server tests already cover the authorization logic this would exercise.
- [~] T022 [P] [US3] Playwright e2e: GM grants a player control of an additional token (e.g. a summoned creature); confirm the player can now drag that token too, identically to their primary (quickstart.md Scenario 3 step 4) — written, same `test.skip`-ed test as T021, not live-verified.
- [~] T023 [P] [US3] Playwright e2e: a player changes their primary token's photo — visible to the GM and other players; the same player has no "create token" control anywhere in the UI (FR-009a, FR-009b; quickstart.md Scenario 3 step 5) — written, same `test.skip`-ed test as T021, not live-verified.

**T021-T023's combined test is `test.skip`-ed, not passing.** Two real bugs were found and fixed while writing it (both kept, independent of the test's outcome): (1) `TokenPanel.tsx`'s primary-token checkbox had no optimistic local state — purely server-round-trip-controlled, so a real user's click would visibly revert before snapping to the new value; fixed with an optimistic update + rollback-on-failure. (2) The test's `assignTokenOwnership` helper called `.blur()` on the owner-input field, which moves focus to `document.body` — Radix `Popover.Content`'s default outside-focus dismissal treats that as "focus left the popover" and closes it before the primary checkbox can be reached; fixed by using `.press("Tab")` instead. After both fixes, the test still hangs the full 300s timeout on `primaryCheckbox.check()` — a second, unconfirmed popover-dismissal trigger remains (candidates: `refresh()`'s list re-render remounting `Popover.Root`, or a timing interaction with the new optimistic-update path). This needs React DevTools/render-log instrumentation to isolate, not more black-box Playwright reruns — see the follow-up spec this was folded into.
