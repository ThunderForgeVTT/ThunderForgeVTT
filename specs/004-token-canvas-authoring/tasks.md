...
- [ ] T020 [US2] Add resize/rotate controls to a new `apps/web/src/components/canvas-tools/TokenTool/TokenTool.tsx`, mirroring `WallTool.tsx`'s `worldStore.dispatch`/`Panel` conventions, mounted GM-only via the existing `isSceneOwner && sceneId` guard in `apps/web/src/pages/world/WorldPage.tsx` (mirroring line ~506's `WallTool` mount) — NOT DONE: no UI panel exists for the keyboard shortcuts (there's nothing to click, only `]`/`[`/`,`/`.` to press while a token is selected) — a real, separate follow-up gap (discoverability), not merely cosmetic.

**Checkpoint**: User Story 2 functionally complete and live-verified (T013/T014) via the documented keyboard-shortcut deviation; T015 (non-GM handle-hiding, live) and T020 (a UI panel/discoverability for the shortcuts) remain open.
