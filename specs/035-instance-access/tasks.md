...
- [~] T056 [P] **Deferred to the playtest pass, not dropped.** Walk [quickstart.md](./quickstart.md) Scenarios A and B by hand with a provider genuinely configured, and tick its Definition of Done. **Scenario A step 5 is the one that matters**: the automated suite proves the gate refuses at the decision level and proves the local route end to end, but the e2e harness has no OAuth provider to hand, so no automated test exercises a real provider handshake against a closed instance. Until a person runs that step, FR-006 is proven by unit test and by construction — the gate is one function and `grep ensure_admission_allowed` finds both call sites — but not by observation. Registered in [spec 032's deferred-manual-pass table](../032-pack-architecture/tasks.md#manual-passes-deferred-to-the-playtest)
