...
- [ ] T063 Run the full manual verification checklist at the bottom of quickstart.md (all 9 scenarios) and confirm every item is checked — a second pass wired most of the previously-missing UI into apps/web: `ActorDetailPage.tsx` now mounts `packs/systems/genie/web`'s `CharacterSheet`/`ConditionTrack` (new `GenieActorSheet`), and `WorldStagingPage.tsx` now mounts `SessionWishPool`/`SessionClocks` (new `GenieSessionPanel` + `useGenieSession`/`api/genieSession.ts`, since the backend's `genieSession`/mutations, already fully implemented and tested, had zero frontend caller before this). See quickstart.md's checklist for full per-scenario detail; in summary: **Scenarios 1, 2, 3, 4, 5, 7, 8, and 9 are now genuinely verified end-to-end** (Scenario 8 partially — Wish Pool + Doom/Puzzle Clocks, not resource trading; see below). Scenario 3 (NPC size category → token scale) turned out to have no app bug at all — it was three separate, now-fixed bugs in `genie-npc-and-items.spec.ts` itself (a retry loop that reset its own in-flight fetch every iteration, a stray `Escape` keypress that closed the whole token panel instead of just its dialog, and a regex — `/\b4\b/` — that could never match "4x"). Scenario 9 (win/loss) surfaced a real app bug, now fixed: `useGenieSession`'s `advancePuzzleClock`/`spendResourceOnPuzzleClock` refetched `genieSession(worldId)` after mutating, but that query only ever returns the *active* session by design — a mutation that just won the session made the refetch come back `null` and blank the whole panel. Now merges the mutation's own response into local state instead. Scenario 6 (Wish Points level-up) remains unreachable — Genie has no `level` concept in its data model at all yet, not just a missing UI. Resource trading (`SessionResourceTrade`) was deliberately left unmounted: the backend has no query to list a player's pending incoming trade proposals (only point mutations to propose/accept one), so there's no real data source for that component without either a new backend query or a live subscription transport — building it against fabricated data was rejected as out of scope. The two-real-connected-clients requirement for Scenarios 8-9 also remains unmet — no live subscription transport exists client-side anywhere in this app yet. Still open pending those.
