...
- [ ] T059 Manual: run quickstart.md Scenario 7 — a complete combat encounter using only the Genie system pack (no other system pack loaded), covering a Manifestation roll, a scene-topology switch, and a condition applied and cleared — all four legs it needs (staging an NPC, a Manifestation roll, a scene-topology switch, a condition applied/cleared) now individually pass end-to-end in the real browser (`apps/web/e2e/genie-manifestation-roll.spec.ts`, `genie-scene-topology.spec.ts`, `genie-conditions.spec.ts`, `genie-npc-and-items.spec.ts`), with no known blocker remaining. Left open only because they haven't been re-run together as one combined encounter spec.
...
- [ ] T063 Run the full manual verification checklist at the bottom of quickstart.md (all 9 scenarios) and confirm every item is checked — a second pass wired most of the previously-missing UI into apps/web: `ActorDetailPage.tsx` now mounts `packs/systems/genie/web`'s `CharacterSheet`/`ConditionTrack` (new `GenieActorSheet`), and `WorldStagingPage.tsx` now mounts `SessionWishPool`/`SessionClocks` (new `GenieSessionPanel` + `useGenieSession`/`api/genieSession.ts`, since the backend's `genieSession`/mutations, already fully implemented and tested, had zero frontend caller before this). See quickstart.md's checklist for full per-scenario detail; in summary: **Scenarios 1, 2, 3, 4, 5, and 8 are now genuinely verified end-to-end** (Scenario 8 partially — Wish Pool + Doom Clock, not resource trading or a win/loss run; see below). Scenario 3 (NPC size category → token scale) turned out to have no app bug at all — it was three separate, now-fixed bugs in `genie-npc-and-items.spec.ts` itself (a retry loop that reset its own in-flight fetch every iteration, a stray `Escape` keypress that closed the whole token panel instead of just its dialog, and a regex — `/\b4\b/` — that could never match "4x"). Scenario 6 (Wish Points level-up) remains unreachable — Genie has no `level` concept in its data model at all yet, not just a missing UI. Scenario 7 (combined encounter) has all four of its legs individually passing now but hasn't been re-run as one combined spec. Scenario 9 (win/loss) is reachable in principle (the mutations are wired) but wasn't driven to an actual win/loss state by an automated test. Resource trading (`SessionResourceTrade`) was deliberately left unmounted: the backend has no query to list a player's pending incoming trade proposals (only point mutations to propose/accept one), so there's no real data source for that component without either a new backend query or a live subscription transport — building it against fabricated data was rejected as out of scope. Still open.
