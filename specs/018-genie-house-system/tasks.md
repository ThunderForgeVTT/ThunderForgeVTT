...
- [ ] T063 Run the full manual verification checklist at the bottom of quickstart.md (all 9 scenarios) and confirm every item is checked — **all 9 scenarios are now genuinely verified end-to-end.** A second pass wired most of the previously-missing UI into apps/web (`GenieActorSheet` on `ActorDetailPage.tsx`, `GenieSessionPanel` on `WorldStagingPage.tsx`); spec 019 (`specs/019-genie-leveling-and-trading/`) closed the remaining two gaps — Scenario 6 (added `trait_data.level` + a CharacterSheet "Resources" tab, since Genie had no level concept in its data model at all before) and Scenario 8's resource trading (added the missing `genieTradeProposals` query, and fixed a real authorization bug where `caller_controls_actor` only checked direct ownership, silently breaking every Session Resource action for a player who claimed rather than created their character — the normal onboarding path). Scenario 3 (NPC size category → token scale) turned out to have no app bug at all — three now-fixed test bugs in `genie-npc-and-items.spec.ts`. Scenario 9 (win/loss) surfaced a real app bug, now fixed: `useGenieSession`'s puzzle-clock mutations refetched `genieSession(worldId)`, which only ever returns the *active* session by design — winning blanked the panel instead of showing "Session won." The two-real-connected-clients requirement for Scenarios 8-9 remains unmet — no live subscription transport exists client-side anywhere in this app yet. Still open pending that.
