...
- [ ] T059 Manual: run quickstart.md Scenario 7 — a complete combat encounter using only the Genie system pack (no other system pack loaded), covering a Manifestation roll, a scene-topology switch, and a condition applied and cleared — the Manifestation roll, scene-topology switch, AND condition legs now each individually pass end-to-end in the real browser (`apps/web/e2e/genie-manifestation-roll.spec.ts`, `genie-scene-topology.spec.ts`, `genie-conditions.spec.ts` — the last added once `ActorDetailPage.tsx`'s `GenieActorSheet` wired `packs/systems/genie/web`'s `CharacterSheet`/`ConditionTrack` in). Left open only because the three legs haven't been re-run together as one combined encounter spec, and because staging an NPC as part of that encounter still runs into the pre-existing, unrelated Scenario 3 NPC-scale bug (see T063).
