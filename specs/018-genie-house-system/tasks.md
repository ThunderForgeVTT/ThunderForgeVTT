...
- [X] T043 [US7] Manual: run quickstart.md Scenarios 8 and 9 in a running dev instance with at least two real connected clients (not a single-client simulation), per Constitution Principle V — genuinely verified: a frontend `graphql-ws` client now drives the backend's `worldEventsCreated` subscription (`apps/web/src/engine/world/sync/subscriptionClient.ts`, wired into `useGenieSession.ts`), confirmed via real WebSocket frame inspection across two separate authenticated clients (one client's action produces a real event-tagged frame on the other's connection within seconds, no reload).
...
- [X] T059 Manual: run quickstart.md Scenario 7 — a complete combat encounter using only the Genie system pack (no other system pack loaded), covering a Manifestation roll, a scene-topology switch, and a condition applied and cleared — confirmed: `apps/web/e2e/genie-full-encounter.spec.ts` runs all four legs (staging an NPC, a condition applied and cleared, a Manifestation roll, a scene-topology switch) together end-to-end in the real browser and passes.
...
- [X] T063 Run the full manual verification checklist at the bottom of quickstart.md (all 9 scenarios) and confirm every item is checked — **all 9 scenarios are now genuinely verified end-to-end, including the two-real-connected-clients requirement for Scenarios 8-9.** A second pass wired most of the previously-missing UI into apps/web (`GenieActorSheet` on `ActorDetailPage.tsx`, `GenieSessionPanel` on `WorldStagingPage.tsx`); spec 019 (`specs/019-genie-leveling-and-trading/`) closed Scenario 6 (added `trait_data.level` + a CharacterSheet "Resources" tab, since Genie had no level concept in its data model at all before) and Scenario 8's resource trading (added the missing `genieTradeProposals` query, and fixed a real authorization bug where `caller_controls_actor` only checked direct ownership, silently breaking every Session Resource action for a player who claimed rather than created their character — the normal onboarding path). A further pass closed live cross-client sync: the backend's real-time stack (Postgres listener → broadcast channel → `worldEventsCreated` GraphQL subscription → axum WebSocket route) was already fully built and mounted but nothing in apps/web ever opened it — added a `graphql-ws` client (`apps/web/src/engine/world/sync/subscriptionClient.ts`), wired into `useGenieSession.ts` and `WorldPage.tsx`'s canvas-primitive sync, and fixed a real pre-existing authorization gap (`worldEventsCreated` had no per-world membership check at all — any authenticated user could subscribe to any world's events). Scenario 3 (NPC size category → token scale) turned out to have no app bug at all — three now-fixed test bugs in `genie-npc-and-items.spec.ts`. Scenario 9 (win/loss) surfaced a real app bug, now fixed twice over: `useGenieSession`'s puzzle-clock mutations, and separately its live-sync NOTIFY handler, both called a `refetch()` that blindly overwrote a concluded (won/lost) session with the `null` `genieSession(worldId)` returns once a session is no longer active — `refetch()` itself now guards against that.
