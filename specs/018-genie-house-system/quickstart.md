...
- [ ] All 9 scenarios above pass in a running dev instance, with Scenarios 8-9 verified across at least two real connected clients (not a single-client simulation). **8 of 9 scenarios now genuinely verified** against a real dev instance with real Playwright browser automation (`apps/web/e2e/genie-*.spec.ts`). Only Scenario 6 remains open (no `level` concept exists in Genie's data model yet), and the two-real-client requirement for 8-9 is still unmet — no live subscription transport exists client-side anywhere in this app. Per-scenario state:
