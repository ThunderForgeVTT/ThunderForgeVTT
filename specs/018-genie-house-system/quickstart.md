...
- [ ] All 9 scenarios above pass in a running dev instance, with Scenarios 8-9 verified across at least two real connected clients (not a single-client simulation). **Partially run this pass**, against a real dev instance with real Playwright browser automation (new specs in `apps/web/e2e/`: `genie-manifestation-roll.spec.ts`, `genie-scene-topology.spec.ts`, `genie-npc-and-items.spec.ts`). Per-scenario state:
  ...
  - [ ] Scenario 3 (NPC size category → token footprint) — attempted and found to be a **real, confirmed bug**, not a test issue: `TokenPanel`'s NPC-scale resolution (spec 018 T047) reads an actor's `trait_data.size_category` via the client-side RxDB `world_actor_system_data` collection, but nothing in the running app ever populates that collection from the server (no pull/push replication is registered for it in `database.ts`, despite the collection's own header comment claiming "Automatic replication via GraphQL subscriptions"; the server also exposes no query field to read it back, only a mutation and a subscription). A diminutive NPC's token defaulted to scale 1x (not 0.5x) even after a full page reload. Documented as `test.fail()` in `genie-npc-and-items.spec.ts` rather than skipped. Also: no UI exists anywhere to set an NPC's `size_category` in the first place (`trait_data` was set via a direct GraphQL mutation for this test).
  - [ ] Scenario 4 (conditions on sheet + token) — NOT reachable: no UI anywhere in apps/web applies or displays a condition (`ConditionTrack.tsx` exists in `packs/systems/genie/web` but is never imported by apps/web).
  ...
  - [ ] Scenario 6 (Wish Points level-up) — NOT reachable: no level-up UI exists anywhere in apps/web (no page/component references a character `level` field or `wishPoints`/`max_wish_points`).
  - [ ] Scenario 7 (full combat encounter, Genie-only) — NOT fully reachable: the Manifestation roll, scene-topology switch legs both pass for real (see Scenarios 1-2 above), but the condition leg (Scenario 4) has no UI path at all, so a complete encounter per SC-002's definition could not be run end-to-end.
  - [ ] Scenario 8 (session loop live-sync across two clients) — NOT reachable: no UI anywhere in apps/web starts a Genie session, shows the Session Wish Pool/Doom Clock/Puzzle Clocks, or exposes resource trading. `apps/web/src/engine/world/sync/genieSession.ts` exists but is not imported/wired into any route or component. This matches the flagged, broader finding that no live GraphQL subscription transport exists client-side anywhere in this app yet.
  - [ ] Scenario 9 (session win/loss) — NOT reachable, same reason as Scenario 8.
