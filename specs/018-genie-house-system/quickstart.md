...
- [X] All 9 scenarios above pass in a running dev instance, with Scenarios 8-9 verified across at least two real connected clients (not a single-client simulation). **9 of 9 scenarios now genuinely verified**, including the two-real-client requirement: a frontend `graphql-ws` client (`apps/web/src/engine/world/sync/subscriptionClient.ts`) now drives the backend's `worldEventsCreated` GraphQL subscription (which — along with its Postgres-listener → broadcast-channel → axum WebSocket transport — was already fully built and mounted server-side, just never opened from any client), wired into `useGenieSession.ts` and `WorldPage.tsx`. Confirmed live end-to-end via real WebSocket frame inspection: a token created on one authenticated client's page arrives as a real `eventCode`-tagged frame on a second client's connection within seconds, no reload — and a non-member's subscription attempt is correctly rejected (`worldEventsCreated` had no per-world authorization at all before this pass; fixed alongside wiring it up for real use). Per-scenario state:
