...
- [ ] All 9 scenarios above pass in a running dev instance, with Scenarios 8-9 verified across at least two real connected clients (not a single-client simulation). **Partially run this pass**, against a real dev instance with real Playwright browser automation (new specs in `apps/web/e2e/`: `genie-manifestation-roll.spec.ts`, `genie-scene-topology.spec.ts`, `genie-npc-and-items.spec.ts`). Per-scenario state:
  ...
  - [ ] Scenario 3 (NPC size category → token footprint) — **still a real, confirmed bug**, not a test issue (re-confirmed after the Genie UI wiring pass below, unaffected by it): `TokenPanel`'s NPC-scale resolution (spec 018 T047) reads an actor's `trait_data.size_category`; a diminutive NPC's token still defaults to scale 1x (not 0.5x) even after a full page reload. Documented as a failing assertion in `genie-npc-and-items.spec.ts`. Also: no UI exists anywhere to set an NPC's `size_category` in the first place (`trait_data` was set via a direct GraphQL mutation for this test).
  ...
  - [ ] Scenario 6 (Wish Points level-up) — still NOT reachable: `packs/systems/genie/web`'s `CharacterSheet` is now mounted (see Scenario 4), but Genie has no `level` concept at all today (it's explicitly class-less per spec.md's Assumptions) — no page/component references a character `level` field or `wishPoints`/`max_wish_points`, and this scenario can't be exercised until that mechanic exists.
  - [ ] Scenario 7 (full combat encounter, Genie-only) — the Manifestation roll, scene-topology switch, and condition legs (Scenarios 1, 2, 4) now all individually pass for real; the remaining blocker is only Scenario 3's pre-existing NPC-scale bug (staging an NPC itself works fine — creating and placing it — just its footprint scale doesn't reflect `size_category`), not a missing UI path. Not re-run as one combined encounter spec this pass.
  - [X] Scenario 8 (Session Wish Pool + Doom/Puzzle Clocks) — genuinely verified end-to-end after adding a full frontend GraphQL client (`apps/web/src/api/genieSession.ts`, `useGenieSession.ts`) and a `GenieSessionPanel` on the GM staging page: a GM started a session, spent a wish (pool 3→2), and advanced the Doom Clock (0→1) (`apps/web/e2e/genie-session-loop.spec.ts`). **Resource trading is still NOT wired**: the backend only exposes point mutations (`proposeResourceTrade`/`acceptResourceTrade`), not a query to list a player's pending incoming proposals, so `SessionResourceTrade` has no real data source for its `incomingProposals` prop without either a new backend query or a live subscription transport (neither exists) — deliberately left unmounted rather than built against fabricated data. Also still not verified across two real connected clients — no live subscription transport exists client-side yet (same gap noted for Scenario 9), so both browser tabs would need to separately poll/refetch today.
  - [ ] Scenario 9 (session win/loss) — reachable in principle now that session/clock mutations are wired (see Scenario 8), but not driven to an actual win/loss state by an automated test this pass.
