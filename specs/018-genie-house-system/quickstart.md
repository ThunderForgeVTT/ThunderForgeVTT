...
- [ ] All 9 scenarios above pass in a running dev instance, with Scenarios 8-9 verified across at least two real connected clients (not a single-client simulation). **Partially run this pass**, against a real dev instance with real Playwright browser automation (new specs in `apps/web/e2e/`: `genie-manifestation-roll.spec.ts`, `genie-scene-topology.spec.ts`, `genie-npc-and-items.spec.ts`). Per-scenario state:
  ...
  - [ ] Scenario 7 (full combat encounter, Genie-only) — all four legs it needs (staging an NPC, a Manifestation roll, a scene-topology switch, a condition applied/cleared — Scenarios 1-4) now individually pass for real, no known blocker remains; just not yet re-run as one combined encounter spec.
