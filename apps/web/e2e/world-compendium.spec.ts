...
test.describe("Compendium shell: tabs, NPC browse/search, row-select preview", () => {
  test("loads inside app chrome with NPCs tab selected, real data, search, and preview", async ({
    ...
    await expect(
      page.getByText(`No NPCs match "zzz-no-match-zzz"`),
    ).toBeVisible();
    ...
    // assertion that used to live here ("Abilities — coming soon") described
  ...
  test("a non-member of the world cannot see its NPC data via the Compendium", async ({
    ...
    const worldId = await registerAndCreateWorld(
      page,
      `E2E Compendium Private ${uniqueSuffix()}`,
    );
    ...
    try {
      ...
      await expect(outsiderPage.getByTestId("npc-catalog-table")).toHaveCount(
        0,
      );
    } finally {
...
test.describe("US1: DM adds and edits an NPC from the Compendium", () => {
  test("DM adds an NPC via the Compendium, edits its description, and sees the update", async ({
    ...
    const worldId = await registerAndCreateWorld(
      page,
      `E2E Compendium DM ${uniqueSuffix()}`,
    );
  ...
  test("Player sees identical browse/search/preview, no Add NPC, and no Edit on a Viewer-only NPC", async ({
    ...
    try {
      ...
      await playerPage
        ...
        .getByText(npcName)
        .click();
      await expect(playerPage.getByTestId("actor-preview-panel")).toContainText(
        npcName,
      );
      ...
      await expect(
        playerPage.getByTestId("actor-preview-panel-view"),
      ).toBeVisible();
      await expect(
        playerPage.getByTestId("actor-preview-panel-edit"),
      ).toHaveCount(0);
    } finally {
...
async function addItemFromCompendium(
  page: Page,
  worldId: string,
  name: string,
): Promise<void> {
...
test.describe("Spec 013 US1: DM authors an Item with a description and structured effects", () => {
  test("DM adds an Item via the Compendium, adds effects, and sees them persist", async ({
    page,
  }) => {
    const worldId = await registerAndCreateWorld(
      page,
      `E2E Items DM ${uniqueSuffix()}`,
    );
    ...
    const savedFormulaInput = page.locator(
      '[data-testid^="item-effect-row-"] input[aria-label="Effect formula"]',
    );
    await expect(savedFormulaInput).toHaveValue("3d6", { timeout: 10_000 });
    await expect(
      page.locator(
        '[data-testid^="item-effect-row-"] input[aria-label="Effect target"]',
      ),
  ...
  test("rejects an empty effect formula without persisting it", async ({
    page,
  }) => {
    const worldId = await registerAndCreateWorld(
      page,
      `E2E Items Validation ${uniqueSuffix()}`,
    );
...
test.describe("Spec 013 US4: a Player browses the Items tab with the same read access, minus DM controls", () => {
  test("Player sees identical browse/preview, no Add Item, and no Edit on a Viewer-only Item", async ({
    ...
    try {
      ...
      await expect(playerPage.getByTestId("item-catalog-table")).toContainText(
        itemName,
      );
      await playerPage
        .getByTestId("item-catalog-table")
        .getByText(itemName)
        .click();
      await expect(playerPage.getByTestId("item-preview-panel")).toContainText(
        itemName,
      );
      ...
      await expect(
        playerPage.getByTestId("item-preview-panel-view"),
      ).toBeVisible();
      await expect(
        playerPage.getByTestId("item-preview-panel-edit"),
      ).toHaveCount(0);
      await expect(
        playerPage.locator('[data-testid^="item-catalog-edit-"]'),
      ).toHaveCount(0);
    } finally {
      await playerContext.close();
    }
