...
test.describe("Spec 020 User Story 2: NPC shop sells items for Session Resources or barter", () => {
  test("a player buys a resource-priced listing and a barter listing from a real NPC shop", async ({
    browser,
  }) => {
    ...
    await graphql(
      gmPage,
      `
        mutation ($input: AddItemToInventoryInput!) {
    ...
    await gmPage
      ...
      .selectOption({ label: "Shop Customer" });
    await gmPage
      .locator(
    ...
    await expect
      .poll(
        async () => {
          ...
          return (
            holdings.data.genieResourceHoldings.find(
              (h) => h.resourceType === "insight",
            )?.quantity ?? 0
          );
        },
        { timeout: 10_000 },
    ...
    await expect(gmPage.getByTestId("genie-shop-panel")).toBeVisible({
      timeout: 10_000,
    });
    await gmPage
      .getByTestId("shop-new-listing-item-select")
      .selectOption({ label: "Rusty Lantern" });
    await gmPage.locator('[aria-label="Price amount"]').fill("2");
    await gmPage.getByTestId("shop-create-listing-button").click();
    await expect(gmPage.locator('[data-testid^="shop-listing-"]')).toHaveCount(
      1,
      { timeout: 10_000 },
    );
    ...
    await gmPage
      .getByTestId("shop-new-listing-item-select")
      .selectOption({ label: "Traveling Dagger" });
    await gmPage.getByText("Item barter").click();
    await gmPage
      .locator('[aria-label="Required barter item"]')
      .selectOption({ label: "Sealed Flask" });
    await gmPage.getByTestId("shop-create-listing-button").click();
    await expect(gmPage.locator('[data-testid^="shop-listing-"]')).toHaveCount(
      2,
      { timeout: 10_000 },
    ...
    await expect(playerPage.getByTestId("genie-shop-panel")).toBeVisible({
      timeout: 10_000,
    });
    const lanternRow = playerPage.locator('[data-testid^="shop-listing-"]', {
      hasText: "Rusty Lantern",
    });
    ...
    const daggerRow = playerPage.locator('[data-testid^="shop-listing-"]', {
      hasText: "Traveling Dagger",
    });
    ...
    expect(
      buyerItems.some((e) => e.itemId === lanternId && e.quantity >= 1),
    ).toBe(true);
    expect(
      buyerItems.some((e) => e.itemId === daggerId && e.quantity >= 1),
    ).toBe(true);
    ...
    expect(
      npcInventory.data.actorInventory.find((e) => e.itemId === flaskId)
        ?.quantity ?? 0,
    ).toBe(1);
