...
function normalizeOAuthResponse(
  payload: OAuthResponsePayload | null,
): OAuthActionResponse {
  return {
    status: payload?.status ?? "unknown",
    message: payload?.message ?? "Request completed",
...
async function expectAuthResponse(
  ...
  const payload = normalizeAuthResponse(
    await readJson<AuthResponsePayload>(response),
  );
...
async function expectOAuthResponse(
  response: Response,
): Promise<OAuthActionResponse> {
  const payload = normalizeOAuthResponse(
    await readJson<OAuthResponsePayload>(response),
  );
...
async function expectDeleteResponse(
  response: Response,
...
export async function exportUserData(
  ...
  const response = await fetch(
    `${API_BASE}/user/data/export?format=${format}`,
    {
      credentials: "same-origin",
    },
  );
...
export function basicLogin(
  ...
  return fetch(`${API_BASE}/authentication/basic`, {
    ...
    async (response) => normalizeAuthResponse(await readJson(response)).message,
  );
}
