...
async function expectMessage(response: Response): Promise<string> {
  const message = await readAuthMessage(response);
...
export function setupBasic(
  ...
  return fetch(`${API_BASE}/authentication/setup/basic`, {
    method: "POST",
    credentials: "same-origin",
    headers: {
