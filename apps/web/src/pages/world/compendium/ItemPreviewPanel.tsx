 ...
 */
export function ItemPreviewPanel({ worldId, item, onClose }: ItemPreviewPanelProps) {
  if (!item) {
    return (
      <Card className="grid place-items-center p-6 text-center" data-testid="item-preview-panel-empty">
    ...
    <Card className="grid gap-4 p-5" data-testid="item-preview-panel">
      ...
      {item.effects.length > 0 ? (
        <div className="grid gap-1">
          <p className="text-xs font-semibold tracking-widest text-muted-foreground uppercase">Effects</p>
      ...
      <div className="flex gap-2">
        <Button asChild variant="secondary" size="sm" data-testid="item-preview-panel-view">
          <Link to={`/world/${worldId}/item/${item.id}/view`}>View</Link>
        </Button>
        {canEdit ? (
          <Button asChild variant="secondary" size="sm" data-testid="item-preview-panel-edit">
