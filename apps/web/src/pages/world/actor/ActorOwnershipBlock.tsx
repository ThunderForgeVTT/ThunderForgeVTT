...
/**
 * Spec 010 (US3, FR-013–FR-017): the DM-only "ownership block" editor —
 * lists every world member plus the DM, shows each one's explicit
 * Viewer/Editor/Owner grant (or "default"), and lets the DM change it.
 * The caller (`ActorDetailPage.tsx`) is responsible for only rendering
...
export function ActorOwnershipBlock({ actorId, worldId, world }: ActorOwnershipBlockProps) {
  ...
  return (
    <Card className="grid gap-3 p-6" data-testid="actor-ownership-block">
      <h3 className="text-lg font-semibold">Ownership block</h3>
      <p className="text-sm text-muted-foreground">
        Only the DM can change who has access to this actor.
      </p>
      ...
      <div className="grid gap-2">
        {subjects.map((subject) => {
          ...
          return (
            <div
              key={subject.userId}
              data-testid={`ownership-row-${subject.userId}`}
