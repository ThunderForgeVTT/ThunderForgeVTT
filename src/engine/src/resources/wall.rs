...
/// Whether the local session is acting as GM (T012: wall edit handles and
/// tool input are GM-only, per data-model.md's Canvas Layer section —
...
/// assigns the real UUID on create; see lib.rs's create_wall handling) —
/// this is deliberately simple rather than a full temp-ID reconciliation
/// system (see WallPlugin doc comment for the rationale).
...
pub struct WallSet {
    ...
    /// (T013) can react to `WallSet` changes without depending on Bevy's
    /// `Changed<T>` filter (which applies to components, not resources
    /// accessed this way) — cleared by the system that consumes it.
    pub dirty: bool,
}
...
/// eventually a full shadow mesh) is done by a separate system that calls
/// this function — see `systems/wall.rs`'s `apply_vision_occlusion`.
