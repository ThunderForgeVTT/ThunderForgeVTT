...
mod tests {
    ...
    impl GameSystem for TestSystem {
        ...
        fn name(&self) -> &'static str { "Test System" }
        fn ability_names(&self) -> Vec<&'static str> { vec![] }
        fn skill_definitions(&self) -> Vec<SkillDefinition> { vec![] }
        fn validate_token(&self, _: &crate::components::Token) -> Result<(), String> { Ok(()) }
        fn calculate_derived_stats(&self, _: &crate::components::Token) -> DerivedStats { 
            DerivedStats::default() 
        }
        fn calculate_movement_cost(&self, distance: f32) -> f32 { distance }
    }
