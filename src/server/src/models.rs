...
// Genie's session rows — eleven structs for its Wish Pool, Doom Clock,
// Puzzle Clocks, holdings, trades and shop listings — stood here. They are
// `packs/systems/genie/server/src/session/models.rs` now: the server has no
// reason to hold a struct for a Doom Clock, and holding one is what made
// this file a place every ruleset eventually adds to (spec 032, ADR-063).
