...
/// Spec 046 FR-016: a unique NPC's tokens place linked. That is only true of a
/// copy if the copy is still unique, so the flag travels with a collection copy
/// and the personal export, in both directions: a copied Boblin places linked,
/// and a copied goblin still places as copies.
#[tokio::test]
async fn an_npcs_uniqueness_travels_with_a_copy_and_an_export() {
    use crate::schema::world_actors;

    let s = source();
    let mut conn = s.state.db_pool.get().expect("connection");
    let boblin = insert_test_actor(&mut conn, s.world_id, s.scene_id, s.owner_id);
    diesel::update(world_actors::table.filter(world_actors::id.eq(boblin)))
        .set((
            world_actors::label.eq("Boblin the goblin"),
            world_actors::is_unique.eq(true),
        ))
        .execute(&mut conn)
        .expect("make it unique");
    drop(conn);

    let code = share_of(
        &s,
        &[("actor", s.actor_id), ("actor", boblin)],
        "Uniqueness",
    )
    .await;
    let receipt = copy_shared_collection_to_world_impl(
    ...
    let mut conn = s.state.db_pool.get().expect("connection");
    let mut unique_copy = |name: &str| -> bool {
        let id = receipt
            .created
            .iter()
            .find(|c| c.member_type == "actor" && c.name == name)
    ...
    assert!(
        unique_copy("Boblin the goblin"),
        "a unique NPC arrives unique"
    );
    assert!(
        !unique_copy("Test Actor"),
        "an ordinary NPC arrives ordinary"
    );
    ...
    assert!(flag(boblin), "the export says Boblin is unique");
    assert!(!flag(s.actor_id), "the export says the ordinary NPC is not");
}
