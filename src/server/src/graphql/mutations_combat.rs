...
impl DownedBy {
    fn from_column(value: Option<&str>) -> Option<Self> {
        match value {
            Some(crate::combat::hit_points::DOWNED_BY_HIT_POINTS) => Some(DownedBy::HitPoints),
            Some(crate::combat::hit_points::DOWNED_BY_GAME_MASTER) => Some(DownedBy::GameMaster),
            _ => None,
        }
    }
}
...
pub async fn update_combatant_impl(
    ...
    let updated = tokio::task::spawn_blocking(move || -> Result<GraphQLCombat, String> {
        ...
        if let Some(active) = input.active {
            // Spec 046 C8: the tracker's Down is the Game Master's, and
            // healing never undoes it; Up clears whatever took it out.
            let downed_by = (!active).then_some(crate::combat::hit_points::DOWNED_BY_GAME_MASTER);
            diesel::update(world_combatants::table.filter(world_combatants::id.eq(combatant_id)))
                .set((
                    world_combatants::active.eq(active),
                    world_combatants::downed_by.eq(downed_by),
                    world_combatants::updated_at.eq(now),
                ))
                .execute(&mut conn)
...
#[cfg(test)]
#[path = "mutations_combat_tests.rs"]
mod tests;
